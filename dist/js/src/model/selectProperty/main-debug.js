"use strict";
define("src/model/selectProperty/main-debug", ["common/jquery-debug", "common/domUtil-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "common/handlerbars-debug", "model/ajax/main-debug", "common/myWidget-debug", "src/model/selectProperty/properties-hbs-debug"], function(require, exports, module) {
    var $ = require("common/jquery-debug"),
        domUtil = require("common/domUtil-debug"),
        Ajax = (require("common/util-debug"), require("common/handlerbars-debug"), require("model/ajax/main-debug")),
        MyWidget = require("common/myWidget-debug"),
        tpl = require("src/model/selectProperty/properties-hbs-debug"),
        SelectProperty = MyWidget.extend({
            ...
            events: {
                "click td": function(e) {
                    ...
                    me.get("parentTr").find('[name="propertyKey"]').val(ele.data("key")), "object" == typeof ele.data("value") ? me.get("parentTr").find('[name="value"]').val(JSON.stringify(ele.data("value"))) : me.get("parentTr").find('[name="value"]').val(ele.data("value")), me.triggerNode.trigger("blur")
                }
            },
            initProps: function() {
                var me = this;
                me.set("parentNode", me.triggerNode.parent()), me.set("parentTr", me.triggerNode.closest("tr"))
            },
            setup: function() {
                var me = this,
                    DOC = $(document);
                domUtil.onChange(me.triggerNode, function() {
                    ...
                    DOC.trigger("click.SelectProperty"), me.delayGetCompanyList(), DOC.on("click.SelectProperty", function() {
                        me.removeDataAndHideList(), DOC.off("click.SelectProperty")
                    })
                }), me.triggerNode.on("click", function(e) {
                    e.stopPropagation()
                }), me.render()
            },
            destroy: function() {
                var me = this;
                domUtil.offChange(me.triggerNode), me.triggerNode.off("click").off("focus"), SelectProperty.superclass.destroy.call(me)
            },
            ...
            delayGetCompanyList: function() {
                var me = this;
                return me.element.show(), me.setCompanyLayout(), clearTimeout(me.timeId), me.timeId = setTimeout(function() {
            ...
            getCompanyListByName: function() {
                ...
                name && name.length > 3 ? new Ajax({
                    ...
                    content.length ? (me.cacheData = content, me.element.html(tpl(content))) : me.removeDataAndHideList()
                }).submit() : me.removeDataAndHideList()
            },
            setCompanyLayout: function() {
                var me = this,
                    pos = me.triggerNode.position();
                me.element.width(me.get("width") || me.triggerNode.outerWidth()), me.element.css("top", pos.top + me.triggerNode.outerHeight() + me.get("top")), me.element.css("left", pos.left + me.get("left"))
            }
        });
