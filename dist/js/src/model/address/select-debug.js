"use strict";
define("src/model/address/select-debug", ["common/jquery-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "common/domUtil-debug", "common/handlerbars-debug", "src/model/address/main-debug", "src/model/address/data-debug", "common/myWidget-debug"], function(require, exports, module) {
    function creatSelect(key) {
        var node = document.createElement("select");
        return $(node).attr("data-address", key), node
    }
    ...
    function renderSelect(key) {
        var me = this,
            data = me["get" + key](),
            node = me["node" + key];
        node && (0 === data.list.length && $(node).hide(), me.get("defaultFirst") || data.list.unshift({
            key: "请选择",
            value: ""
        }), domUtil.selectSerialize(node, data.list), data.focus && (node.value = data.focus))
    }
    var $ = require("common/jquery-debug"),
        domUtil = (require("common/util-debug"), require("common/domUtil-debug")),
        Address = require("src/model/address/main-debug"),
        AddressSelect = Address.extend({
            clssName: "AddressSelect",
            attrs: {
                focus: {
                    setter: function(val) {
                        var me = this;
                        me.focus = val, me.renderSelect()
                    },
                    getter: function() {
            ...
            events: {
                "change [data-address]": function(e) {
                    var me = this,
                        self = $(e.target),
                        val = self.val();
                    me.$("[data-address]").show(), val ? me.set("focus", val) : me.set("focus", self.prev().val() || "000000"), me.get("defaultFirst") && self.next().trigger("change")
                }
            },
            setup: function() {
                var me = this;
                0 === me.$("[data-address]").length ? (me.nodeProvince = creatSelect("province"), me.nodeCity = creatSelect("city"), me.nodeArea = creatSelect("area"), me.element.append(me.nodeProvince, me.nodeCity, me.nodeArea)) : (me.nodeProvince = me.$('[data-address="province"]')[0], me.nodeCity = me.$('[data-address="city"]')[0], me.nodeArea = me.$('[data-address="area"]')[0]), me.set("focus", me.get("focus"))
            },
            renderSelect: function() {
                var me = this,
                    focus = me.get("focus");
                return renderSelect.call(me, "Province"), renderSelect.call(me, "City"), renderSelect.call(me, "Area"), me.get("defaultFirst") ? me.getTrueData().area && me.trigger("change", focus) : me.trigger("change", "000000" === focus ? "" : focus), me
            }
        });
...
"use strict";
define("src/model/address/main-debug", ["common/jquery-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "src/model/address/data-debug", "common/myWidget-debug"], function(require, exports, module) {
    function paddingStr(first, key) {
        return +(first + ("00" + key).slice(-2) + "000000").slice(0, 6)
    }

    function eachKey(cb, first) {
        cb = util.maybeCallback(cb), first = first || "";
        for (var val, key, i = 1; i < 100; i++) key = paddingStr(first, i), val = data[key], val && cb(val, key)
    }
    var util = (require("common/jquery-debug"), require("common/util-debug")),
        data = require("src/model/address/data-debug"),
        MyWidget = require("common/myWidget-debug"),
        Address = MyWidget.extend({
            clssName: "Address",
            attrs: {
                focus: "330100"
            },
            getTrueData: function() {
                var temp, target, me = this,
                    focus = me.get("focus"),
                    list = [];
                for (list.unshift(+focus);
                    (temp = data[list[0]]) && "1" !== (target = temp[1]);) list.unshift(+target);
                return {
                    area: list[2],
                    city: list[1],
                    province: list[0]
            ...
            getProvince: function() {
                var me = this,
                    ...
                    list = [];
                return eachKey(function(val, key) {
                    list.push({
                        key: val[0],
                        value: key
                    })
            ...
            getArea: function() {
                for (var me = this, trueData = me.getTrueData(), province = trueData.province, city = "" + trueData.city, list = [], i = 1; i < 100; i++) eachKey(function(val, key) {
                    val[1] === city && list.push({
                        key: val[0],
                        value: key
                    })
...
"use strict";
define("src/model/address/data-debug", [], function(require, exports, module) {
    return {
        152522: ["阿巴嘎旗", "152500"],
        513231: ["阿坝县", "513200"],
        513200: ["阿坝", "510000"],
