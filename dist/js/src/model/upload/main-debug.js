"use strict";
define("src/model/upload/main-debug", ["alinw/upload/2.1.2/upload-debug", "model/modal/main-debug", "common/myWidget-debug", "common/jquery-debug"], function(require, exports, module) {
    ...
    var nwUpload = require("alinw/upload/2.1.2/upload-debug"),
        ...
        TEMPLATE_ONE = handlerbars.compile(['<span class="file-list JS-target-item fn-VATop" data-id="{{id}}">', '<input type="checkbox" checked="checked" name="UploadFileInput" value="{{id}}" style="display:none;"/><a class="file-list-text fn-btn-link" {{#if imgView}}data-rule="imgView"{{/if}} href="{{url}}" target="_blank" title="{{name}}">{{wrapWord name 20}}</a> <span class="fn-CuPo kuma-icon kuma-icon-close JS-trigger-click-remove" aria-hidden="true"></span>', "</span>"].join("")),
        ...
        Upload = MyWidget.extend({
            ...
            initUpload: function() {
                ...
                return new nwUpload({
                    ...
                    change: function(e) {
                        if (e[0]) {
                            var fileSize = e[0].size;
                            if (fileSize && fileSize > me.get("fileSize")) return Modal.alert(0, "文件大小限制在15M以内");
                            var name = e[0].name,
                            ...
                            me.tempName = name, me.element.append(node), me.uploadToggle(), this.submit()
                        }
                    },
                    ...
                    error: function(e) {
                        e.statusText ? "Request Entity Too Large" === e.statusText ? Modal.alert(0, "文件大小限制在15M以内") : Modal.alert(0, e.statusText) : me.log(e.statusText), me.tempNode.remove(), delete nodeMap[me.tempName]
                    }
                })
            },
            ...
            uploadRenderList: function() {
                ...
                return $.each(list, function() {
                    this.imgView = me.get("imgView")
                }), me.element.append(TEMPLATE_LIST(list)), me.uploadToggle().uploadParseData(), me
            },
            ...
            uploadToggle: function() {
                var me = this,
                    ...
                    size = me.get("size");
                return me.$(".file-list").length >= size ? nwUpload.get("form").hide() : nwUpload.get("form").show(), me
            },
