"use strict";
define("src/model/paginator/main-debug", ["common/myWidget-debug"], function(require, exports, module) {
    ...
    var MyWidget = require("common/myWidget-debug"),
        ...
        template = handlerbars.compile(['<nav class="clearfix">', '<ul class="pagination" style="float:left;">', '<li class="JS-click-prev {{#isEqual nowTarget 1}}disabled{{/isEqual}}" data-label="prev"><a href="javascript:;" aria-label="Previous"><span aria-hidden="true">«</span></a></li>', "{{#each list}}", '<li class="JS-click-active {{#isEqual this ../nowTarget}}active{{/isEqual}} {{#isEqual this "..."}}other{{/isEqual}}" data-now-target="{{this}}"><a href="javascript:;">{{this}}</li>', "{{/each}}", '<li class="JS-click-next {{#isEqual nowTarget totle}}disabled{{/isEqual}}" data-label="next"><a href="javascript:;" aria-label="Next"><span aria-hidden="true">»</span></a></li>', "</ul>", '<div style="float:left;margin-left:5px;margin-top:20px;" class="form-group-sm">', "共 {{count}} 条", ' <input type="text" class="form-control JS-target-page" style="display:inline-block;width:50px;" />', ' <button class="btn btn-primary btn-sm JS-click-jump" style="vertical-align:top;">跳 转</button>', "</div>", "</nav>"].join("")),
        Paginator = MyWidget.extend({
            clssName: "Paginator",
            attrs: {
                totle: 100,
                size: 10,
