"use strict";
define("src/model/caseManagement/main-debug", ["common/jquery-debug", "common/myWidget-debug", "model/multiple/main-debug", "model/imgView/main-debug", "common/handlerbars-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "model/ajax/main-debug", "common/scroller-debug", "model/modal/main-debug", "model/cascadeSelect/main-debug", "common/validator-debug"], function(require, exports, module) {
    function dateToString(date) {
        if (date instanceof Date) {
            ...
            return seconds = seconds < 10 ? "0" + seconds : seconds, "" + year + "-" + month + "-" + day + " " + hour + ":" + minus + ":" + seconds
        }
        return ""
    }
    var $ = require("common/jquery-debug"),
        ...
        CaseManagement = MyWidget.extend({
            ...
            events: {
                'change [name="conclusion"]': function(e) {
                    var me = this,
                        node = $(e.target);
                    "return" === node.val() || "not_accepted" === node.val() ? (me.show("#multiple"), me.disabledFalse(node.next())) : (me.hide("#multiple"), me.disabledTrue(node.next())), "make" === node.val() ? "y" == $('input[name="fromTongdahai"]').val() ? ($('[name="remark"]').val(""), $("#remarkTr").addClass("fn-hide"), $("#buttonTd").html(me.get("buttonCommitSyn"))) : me.show("#caseCodeTr") : "y" == $('input[name="fromTongdahai"]').val() ? ($("#remarkTr").removeClass("fn-hide"), $("#buttonTd").html(me.get("buttonSave"))) : ($("[name='caseCode'], [name='dept'], [name='trialCourt'], [name='clerk']").val(""), $("[name=dept]").trigger("change"), me.hide("#caseCodeTr"))
                },
                "click .JS-trigger-click-save": function() {
                    ...
                    me.valExp.execute(function(isErr, errList) {
                        if (!isErr) {
                            ...
                            if (reasonStr && reasonStr.indexOf("other") >= 0 && !$("textarea[name='remark']").val()) "not_accepted" === conclusion ? Modal.alert(0, '选择其他"不予受理"原因，备注不能为空。') : "return" === conclusion && Modal.alert(0, '选择其他"退回"原因，备注不能为空。');
                            else {
                                ...
                                "make" === conclusion ? Modal.confirm("提醒", '<span style="font-size:13px;font-weight:bold;">立案后不可跳回，确定要立案吗？</span>', function() {
                                    submitAjax.submit()
                                }) : submitAjax.submit()
                            }
                        }
                        ...
                        if (!isErr) {
                            ...
                            if (reasonStr && reasonStr.indexOf("other") >= 0 && !$("textarea[name='remark']").val()) "not_accepted" === conclusion ? Modal.alert(0, '选择其他"不予受理"原因，备注不能为空。') : "return" === conclusion && Modal.alert(0, '选择其他"退回"原因，备注不能为空。');
                                ...
                                var commitSynAction = new Ajax({
                                    ...
                                    param: {
                                        securityCaseId: encodeURIComponent($("input[name='securityCaseId']").val()),
                                        courtId: encodeURIComponent($("input[name='courtId']").val())
                                    },
                                    autoSuccessAlert: !0
                                }).on("ajaxSuccess", function() {
                                    location.reload(), $("#caseManage-form").replaceWith($("#template-caseManage-syn").html())
                                });
                                "make" === conclusion ? Modal.confirm("提醒", '<span style="font-size:13px;font-weight:bold;">立案后不可跳回，确定要立案吗？</span>', function() {
                ...
                'change [name="dept"]': function(e) {
                    var me = this,
                        dept = $(e.target);
                    me.$('[name="deptName"]').val(dept.find("option:selected").text()), me.$('[name="clerkName"]').val(""), me.$('[name="trialCourtName"]').val("")
                },
                "click .JS-trigger-more-information": function(e) {
                    var me = this;
                    me.$("#court-more-information").removeClass("fn-hide"), Scroller.use(".JS-need-scroller"), me.$(e.target).parent().remove()
                }
            },
            initProps: function() {
                var me = this;
                me.templateCaseManage = Handlerbars.compile("#template-caseManage", !0), me.valExp = Validator.use("#caseManage-form")
            },
            setup: function() {
                var me = this;
                if (new ImgView, 0 == me.$("select[name='conclusion']").size() && me.$("#multiple")) {
                    var code = me.$("#multiple").data("reason");
                    null != code && "" != code && me.$("#multiple").html(me.getReasons(code))
                }
                if (me.$("select[name='conclusion']").size() > 0 && new Multiple({
                        trigger: "#multiple",
                        width: 420,
                        data: me.get("multipleMap")
                    }), me.$(".cascadeSelect").size() > 0) {
            ...
            getReasons: function(code) {
                var me = this,
                    description = "";
                if (code)
                    for (var codes = code.split(","), c = 0; c < codes.length; c++) description = "" != description ? description + ", " + me.get("multiple")[codes[c]] : me.get("multiple")[codes[c]];
                return description
            }
        });
