"use strict";
define("src/model/applicationMatters/main-debug", ["common/jquery-debug", "common/myWidget-debug", "model/ajax/main-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "common/domUtil-debug", "common/handlerbars-debug", "common/dialog-debug", "model/upload/main-debug", "common/calendar-debug", "common/validator-debug", "model/modal/main-debug", "src/model/applicationMatters/applicationMatters-hbs-debug", "src/model/applicationMatters/dataTemplate-hbs-debug", "src/model/applicationMatters/paymentCount-hbs-debug", "src/model/applicationMatters/paymentStates-hbs-debug"], function(require, exports, module) {
    function totalAmount(list) {
        var me = this,
            amount = 0;
        list.length && (amount = limit.reduce(list, function(res, val) {
            return res + val.currentAccum
        }, 0)), me.$('[data-trigger="totalAmount"]').each(function() {
            var self = $(this);
            "hidden" === self.prop("type") ? self.val(amount) : self.html(util.formatMoney(amount, 2))
    ...
    function getAllAmount() {
        ...
        me.$('[data-role="applyAmount"]').html(util.formatMoney(allAmount, 2)), me.$('[data-role="applyAmountValue"]').val(allAmount)
    }
    ...
    function formatAmout(data) {
        switch (data.application.requestType) {
            case "creditCard":
                data.application.creditCardAmount = data.application.amount, delete data.application.amount, data.other.creditCardAmount = data.other.amount, delete data.other.amount, data.creditCardAmount = data.amount, delete data.amount;
                break;
            case "payment_other":
                data.application.paymentOtherAmount = data.application.amount, delete data.application.amount, data.other.paymentOtherAmount = data.other.amount, delete data.other.amount, data.paymentOtherAmount = +data.amount, delete data.amount
        }
    }
    var $ = require("common/jquery-debug"),
        ...
        applicationMattersHbs = (require("model/modal/main-debug"), require("src/model/applicationMatters/applicationMatters-hbs-debug")),
        dataTemplateHbs = require("src/model/applicationMatters/dataTemplate-hbs-debug"),
        paymentCountHbs = require("src/model/applicationMatters/paymentCount-hbs-debug"),
        paymentStatesHbs = require("src/model/applicationMatters/paymentStates-hbs-debug"),
        ApplicationMatters = MyWidget.extend({
            ...
            events: {
                'click [data-role="data"]': function(rtv) {
                    var me = this,
                        dialog = Dialog.show(dataTemplateHbs($.extend({
                            url: me.get("url"),
                                ...
                                'click [data-role="submit"]': function() {
                                    ...
                                    validatorExp.execute(function(isError) {
                                        isError || new Ajax({
                                            ...
                                            me.$('[data-targer="creditCardFulldata"]').val("true").blur(), me.$("#noneDataTemplate").html(paymentCountHbs(rtv)), totalAmount.call(me, rtv.billMonthDataVos), dialog.hide()
                                        }).submit()
                                    })
                                },
                                'click [data-role="paymentUpload"]': function() {
                                    ...
                                    validatorExp.execute(function(isError) {
                                        isError || new Ajax({
                                            ...
                                            me.$('[data-targer="paymentFullstates"]').val("true").blur(), me.$("#paymentStates").append(paymentStatesHbs(rtv)), dialog.hide()
                                        }).submit()
                                    })
                                }
                            }
                        }).before("hide", function() {
                            dataUpload.destroy(), validatorExp.destroy()
                        }),
                        dataUpload = Upload.use(dialog.$('[data-widget="upload"]'))[0],
                        validatorExp = Validator.use(dialog.element, '[data-widget="validator"]')
                },
                'change [data-role="typeChange"]': function(e) {
                    var me = this,
                        self = $(e.target);
                    "creditCard" === self.val() ? (me.$('[data-role="creditCard"]').removeClass("fn-hide"), me.$('[data-role="payment_other"]').addClass("fn-hide"), domUtil.disabledTrue(me.$('[data-role="payment_other"]')), domUtil.disabledFalse(me.$('[data-role="creditCard"]'))) : (me.$('[data-role="creditCard"]').addClass("fn-hide"), me.$('[data-role="payment_other"]').removeClass("fn-hide"), domUtil.disabledTrue(me.$('[data-role="creditCard"]')), domUtil.disabledFalse(me.$('[data-role="payment_other"]')))
                },
                "click .JS-trigger-click-remove": function(e) {
                    ...
                    new Ajax({
                        ...
                        target.remove(), me.$("#paymentStates .JS-target-item").length || me.$('[data-targer="paymentFullstates"]').val("")
                    }).submit()
                },
                'click [name="rateType"]': function(e) {
                    var me = this,
                        self = $(e.target);
                    "hundredMarkSystem" == self.val() ? (me.$('[data-role="hundredMarkSystem"]').removeClass("fn-hide"), me.$('[data-role="micrometerSystem"]').addClass("fn-hide")) : (me.$('[data-role="hundredMarkSystem"]').addClass("fn-hide"), me.$('[data-role="micrometerSystem"]').removeClass("fn-hide"))
                },
            ...
            setup: function() {
                var me = this,
                    temp = {
                        creditCard: "payment_other",
                        payment_other: "creditCard"
                    },
                    data = me.get("data"),
                ...
                data = {
                    application: $.extend({
                        ...
                        rateType: "hundredMarkSystem"
                    }, data[0]),
                    other: $.extend({}, data[1]),
                    amount: me.get("amount")
                }, formatAmout(data), me.element.html(applicationMattersHbs(data));
                var table = me.$('[data-role="' + temp[data.application.requestType] + '"]');
                domUtil.disabledTrue(table), table.addClass("fn-hide"), table.find('input[type="text"]').not('[name="businessDate"]').not('[data-targer="paymentFullstates"]').not('[data-targer="creditCardFulldata"]').not("#amoutChangeHandle").val(""), billList.billMonthDataVos.length && (me.$('[data-targer="creditCardFulldata"]').val("true").blur(), data.application && (billList.content = data.application.content), me.$("#noneDataTemplate").html(paymentCountHbs(billList)), totalAmount.call(me, billList.billMonthDataVos)), fileDos.length && (me.$('[data-targer="paymentFullstates"]').val("true").blur(), me.$("#paymentStates").html(paymentStatesHbs(fileDos))), domUtil.onChange(me.$("#amoutChangeHandle"), function() {
                    var self = $(this);
                    setTimeout(function() {
                        self.val();
                        getAllAmount.call(me)
                    ...
                    onSelectDate: function(date) {
                        new Ajax({
                            ...
                            param: {
                                securityCaseId: me.get("pageParam").securityCaseId,
                                businessDate: util.formatData("yyyy-MM-dd", +date)
                            }
                        }).on("ajaxSuccess", function(rtv, msg, con) {
                            me.$("#noneDataTemplate").html(paymentCountHbs(rtv)), totalAmount.call(me, rtv.billMonthDataVos)
                        }).submit()
                    },
                    range: function(date) {
...
define("src/model/applicationMatters/paymentCount-hbs-debug", ["common/handlerbars-debug"], function(require, exports, module) {
    var Handlerbars = require("common/handlerbars-debug"),
        compile = Handlerbars.compile('<div>    <div>        <span>罚息利率：{{rate}}</span>    </div>    <input type="hidden" name="content" value="{{content}}" />    <div class="fn-MT10" style="overflow:auto;width:735px;">        <table class="fn-table fn-table-data fn-table-data-sm" width="1500">            <thead>                <tr>                    <th width="100">账单号</th>                    <th width="100">账单月</th>                    <th width="100">账单日</th>                    <th width="100">还款日</th>                    <th width="100">逾期日</th>                    <th width="150">本月应还+费用</th>                    <th width="100">本月已还</th>                    <th width="150">本月账单分期</th>                    <th width="100">本月减免</th>                    <th width="100">应还罚息</th>                    <th width="100">本月应还</th>                    <th>累计应还</th>                </tr>            </thead>            <tbody>                {{#each billMonthDataVos}}                <tr>                    <td>{{billNo}}</td>                    <td>{{formatData \'yyyy/MM\' billMonth}}</td>                    <td>{{formatData \'yyyy/MM/dd\' billDate}}</td>                    <td>{{formatData \'yyyy/MM/dd\' repayDate}}</td>                    <td>{{formatData \'yyyy/MM/dd\' overDate}}</td>                    <td>                        <a href="/suit/start/paymentOrder/billDataDetail.htm?loanId={{loanId}}&billMonth={{formatData \'yyyy/MM\' billMonth}}&fieldName=currentPrincipal" target="_blank" class="fn-color-047dc6">{{currentPrincipal}}</a>                    </td>                    <td>                        <a href="/suit/start/paymentOrder/billDataDetail.htm?loanId={{loanId}}&billMonth={{formatData \'yyyy/MM\' billMonth}}&fieldName=paidBalance" target="_blank" class="fn-color-047dc6">{{paidBalance}}</a>                    </td>                    <td>                        <a href="/suit/start/paymentOrder/billDataDetail.htm?loanId={{loanId}}&billMonth={{formatData \'yyyy/MM\' billMonth}}&fieldName=billInstalment" target="_blank" class="fn-color-047dc6">{{billInstalment}}</a>                    </td>                    <td>                        <a href="/suit/start/paymentOrder/billDataDetail.htm?loanId={{loanId}}&billMonth={{formatData \'yyyy/MM\' billMonth}}&fieldName=reductable" target="_blank" class="fn-color-047dc6">{{reductable}}</a>                    </td>                    <td>                        <a href="/suit/start/paymentOrder/billDataDetail.htm?loanId={{loanId}}&billMonth={{formatData \'yyyy/MM\' billMonth}}&fieldName=currentPenalty" target="_blank" class="fn-color-047dc6">{{currentPenalty}}</a>                    </td>                    <td>                        <a href="/suit/start/paymentOrder/billDataDetail.htm?loanId={{loanId}}&billMonth={{formatData \'yyyy/MM\' billMonth}}&fieldName=currentBalance" target="_blank" class="fn-color-047dc6">{{currentBalance}}</a>                    </td>                    <td>                        <a href="/suit/start/paymentOrder/billDataDetail.htm?loanId={{loanId}}&billMonth={{formatData \'yyyy/MM\' billMonth}}&fieldName=currentAccum" target="_blank" class="fn-color-047dc6">{{currentAccum}}</a>                    </td>                </tr>                {{/each}}            </tbody>        </table>    </div></div>');
    return compile.source = '<div>    <div>        <span>罚息利率：{{rate}}</span>    </div>    <input type="hidden" name="content" value="{{content}}" />    <div class="fn-MT10" style="overflow:auto;width:735px;">        <table class="fn-table fn-table-data fn-table-data-sm" width="1500">            <thead>                <tr>                    <th width="100">账单号</th>                    <th width="100">账单月</th>                    <th width="100">账单日</th>                    <th width="100">还款日</th>                    <th width="100">逾期日</th>                    <th width="150">本月应还+费用</th>                    <th width="100">本月已还</th>                    <th width="150">本月账单分期</th>                    <th width="100">本月减免</th>                    <th width="100">应还罚息</th>                    <th width="100">本月应还</th>                    <th>累计应还</th>                </tr>            </thead>            <tbody>                {{#each billMonthDataVos}}                <tr>                    <td>{{billNo}}</td>                    <td>{{formatData \'yyyy/MM\' billMonth}}</td>                    <td>{{formatData \'yyyy/MM/dd\' billDate}}</td>                    <td>{{formatData \'yyyy/MM/dd\' repayDate}}</td>                    <td>{{formatData \'yyyy/MM/dd\' overDate}}</td>                    <td>                        <a href="/suit/start/paymentOrder/billDataDetail.htm?loanId={{loanId}}&billMonth={{formatData \'yyyy/MM\' billMonth}}&fieldName=currentPrincipal" target="_blank" class="fn-color-047dc6">{{currentPrincipal}}</a>                    </td>                    <td>                        <a href="/suit/start/paymentOrder/billDataDetail.htm?loanId={{loanId}}&billMonth={{formatData \'yyyy/MM\' billMonth}}&fieldName=paidBalance" target="_blank" class="fn-color-047dc6">{{paidBalance}}</a>                    </td>                    <td>                        <a href="/suit/start/paymentOrder/billDataDetail.htm?loanId={{loanId}}&billMonth={{formatData \'yyyy/MM\' billMonth}}&fieldName=billInstalment" target="_blank" class="fn-color-047dc6">{{billInstalment}}</a>                    </td>                    <td>                        <a href="/suit/start/paymentOrder/billDataDetail.htm?loanId={{loanId}}&billMonth={{formatData \'yyyy/MM\' billMonth}}&fieldName=reductable" target="_blank" class="fn-color-047dc6">{{reductable}}</a>                    </td>                    <td>                        <a href="/suit/start/paymentOrder/billDataDetail.htm?loanId={{loanId}}&billMonth={{formatData \'yyyy/MM\' billMonth}}&fieldName=currentPenalty" target="_blank" class="fn-color-047dc6">{{currentPenalty}}</a>                    </td>                    <td>                        <a href="/suit/start/paymentOrder/billDataDetail.htm?loanId={{loanId}}&billMonth={{formatData \'yyyy/MM\' billMonth}}&fieldName=currentBalance" target="_blank" class="fn-color-047dc6">{{currentBalance}}</a>                    </td>                    <td>                        <a href="/suit/start/paymentOrder/billDataDetail.htm?loanId={{loanId}}&billMonth={{formatData \'yyyy/MM\' billMonth}}&fieldName=currentAccum" target="_blank" class="fn-color-047dc6">{{currentAccum}}</a>                    </td>                </tr>                {{/each}}            </tbody>        </table>    </div></div>', compile
});
