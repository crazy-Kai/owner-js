"use strict";
define("src/model/selectForCompanyAddress/main-debug", ["common/jquery-debug", "common/domUtil-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "common/handlerbars-debug", "model/ajax/main-debug", "common/myWidget-debug"], function(require, exports, module) {
    function getDataById(id) {
        var data, me = this,
            cacheData = me.cacheData;
        util.breakEachArr(cacheData, function(val) {
            if (id === val.securityId) return data = val, !0
        });
        for (var i in data) data["lassenSuitEntityDo." + i] = data[i], delete data[i];
        return data
    }
    var $ = require("common/jquery-debug"),
        ...
        tpl = Handlerbars.compile(["<ul>", "{{#each this}}", '<li data-id="{{securityId}}">{{companyName}}</li>', "{{/each}}", "</ul>"].join("")),
        SelectForCompanyAddress = MyWidget.extend({
            ...
            setup: function() {
                ...
                domUtil.onChange(me.triggerNode, function() {
                    ...
                    me.delayGetCompanyList(), DOC.on("click.SelectForCompanyAddress", function() {
                        me.removeDataAndHideList(), DOC.off("click.SelectForCompanyAddress")
                    })
            ...
            destroy: function() {
                var me = this;
                domUtil.offChange(me.triggerNode), me.triggerNode.off("click").off("focus"), SelectForCompanyAddress.superclass.destroy.call(me)
            },
            removeDataAndHideList: function() {
                var me = this;
                delete me.cacheData, me.element.hide()
            },
            delayGetCompanyList: function() {
                var me = this;
                return clearTimeout(me.timeId), me.timeId = setTimeout(function() {
            ...
            getCompanyListByName: function() {
                ...
                name ? new Ajax({
                    ...
                    data.length ? (me.cacheData = data, me.element.show(), me.setCompanyLayout(), me.element.html(tpl(data))) : me.removeDataAndHideList()
