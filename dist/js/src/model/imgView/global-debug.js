"use strict";
define("src/model/imgView/global-debug", ["common/jquery-debug", "common/myWidget-debug", "common/dialog-debug", "model/modal/main-debug", "model/poploading/main-debug", "common/widget-debug", "common/base-debug", "common/class-debug", "common/attrs-debug", "common/aspect-debug", "common/events-debug", "common/limit-debug", "common/limit-dom-debug", "src/model/imgView/imgload-hbs-debug", "common/handlerbars-debug"], function(require, exports, module) {
    function closeCb(img) {
        img.onload = img.onerror = null, $(img).remove(), Poploading.hide()
    }
    var $ = require("common/jquery-debug"),
        ...
        imgloadHbs = require("src/model/imgView/imgload-hbs-debug"),
        ImgView = MyWidget.extend({
            clssName: "ImgView",
            attrs: {
                element: "body",
            ...
            events: {
                "click a": function(e) {
                    ...
                    !target.data("rule") && me.get("rule").test($.trim(target.html())) && (e.preventDefault(), me.imgShow(target.prop("href")))
                }
            },
            imgShow: function(href) {
                ...
                if (href) {
                    var img = new Image;
                    Poploading.show(), img.onload = function() {
                        var width;
                        me.element.append(img), img.clientWidth >= 800 && (width = 800, $(img).removeAttr("height")), Dialog.show(imgloadHbs({
                            href: href,
                            id: href.match(me.get("sliceRep"))[1],
                            width: width
                        })), closeCb(img)
                    }, img.onerror = function() {
                        Modal.alert(0, "您无权限查看或者图片地址错误。"), closeCb(img)
                    }, img.src = href
                }
                return me
            }
