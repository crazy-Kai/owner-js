"use strict";
define("src/common/validator-debug", ["common/jquery-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "validator-debug"], function(require, exports, module) {
    function isIdCard(cardid) {
        var isIdCard2 = /^[1-9]\d{5}(19\d{2}|[2-9]\d{3})((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])(\d{4}|\d{3}X)$/i,
            stard = "10X98765432",
            first = [7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2],
            sum = 0;
        if (!isIdCard2.test(cardid)) return !1;
        var year = cardid.substr(6, 4),
            month = cardid.substr(10, 2),
            day = cardid.substr(12, 2),
            birthday = cardid.substr(6, 8);
        if (birthday != dateToString(new Date(year + "/" + month + "/" + day))) return !1;
        for (var i = 0; i < cardid.length - 1; i++) sum += cardid[i] * first[i];
        var result = sum % 11,
            last = stard[result];
    ...
    function dateToString(date) {
        if (date instanceof Date) {
            ...
            return day = day < 10 ? "0" + day : day, "" + year + month + day
        }
        return ""
    }
    var $ = require("common/jquery-debug"),
        util = require("common/util-debug"),
        ValidatorOld = require("validator-debug"),
        Validator = ValidatorOld.extend({
            clearError: function() {
                var me = this;
                return $(me.items).each(function(val) {
                    this.element.closest(".kuma-form-item").removeClass("kuma-form-item-error")
                }), me
            },
            execute: function(cb) {
                var me = this;
                return me.items ? Validator.superclass.execute.call(me, cb) : cb(null)
            },
            destroy: function() {
                var me = this;
                return me.clearError(), Validator.superclass.destroy.call(me)
            },
            ...
            addItems: function(form, query) {
                ...
                ele.each(function(index1, domEle1) {
                    ...
                    section.find(query || ".JS-target-required").each(function(index2, domEle2) {
                        ...
                        "radio" !== nodeType && "checkbox" !== nodeType || (node = section.find('[name="' + node.prop("name") + '"]')), skipHidden = skipHidden !== !1, me.addItem($.extend({
                            element: node,
                            triggerType: node.data("triggerType"),
                            skipHidden: skipHidden,
                            checkNull: !1,
                            onItemValidate: function(ele) {
                                "text" === nodeType && ele.val($.trim(ele.val()))
                            }
                        }))
                    })
                })
            ...
            Statics: {
                use: function(element, query, elementConfig, queryConfig) {
                    if (element = $(element), element.length) {
                        ...
                        return element.find(query || ".JS-target-required").each(function() {
                            var node = $(this),
                                checkNull = node.data("checkNull"),
                                nodeType = node.prop("type"),
                                skipHidden = node.data("skipHidden"),
                                trim = node.data("trim");
                            "radio" !== nodeType && "checkbox" !== nodeType || (node = element.find('[name="' + node.prop("name") + '"]')), skipHidden = skipHidden !== !1, checkNull = checkNull === !0, trim = trim === !0, validatorExp.addItem($.extend({
                                element: node,
                                triggerType: node.data("triggerType"),
                                skipHidden: skipHidden,
                                checkNull: checkNull,
                                onItemValidate: function(ele) {
                                    ("text" === nodeType || trim && "textarea" === nodeType) && ele.val($.trim(ele.val()))
                                }
                            }, queryConfig))
                        }), validatorExp
                    }
                ...
                oneExecute: function(element, query) {
                    ...
                    return oneValidator && oneValidator.destroy && oneValidator.destroy(), oneValidator = me.use(element, query), oneValidator.execute(function(isErr, errList) {
                        flag = isErr, isErr && util.log(errList)
                    }), me.oneValidator = oneValidator, flag
                }
            }
        });
    return Validator.addRule("cardid", function(options) {
        return isIdCard(options.element.val())
    }, "请填写正确的身份证号"), Validator.addRule("postalcode", function(options) {
        var val = $.trim(options.element.val());
        return !val || /^\d{6}$/.test(val)
    }, "请填写正确的邮政编码"), Validator.addRule("wholeNumber", function(options) {
        var val = $.trim(options.element.val());
        return !val || /^\d+$/.test(val) && +val
    }, "请填写正整数"), Validator.addRule("naturalNumber", function(options) {
        var val = $.trim(options.element.val());
        return !val || /^\d+$/.test(val)
    }, "请填写自然数"), Validator.addRule("tel", function(options) {
        var val = $.trim(options.element.val());
        return !val || /^\d{3,4}-\d{7,8}$/.test(val)
    }, "请填写正确的电话号码(比如：0571-88888888)"), Validator.addRule("mobileOrTel", function(options) {
        var val = $.trim(options.element.val());
        return !val || /^\d{3,4}-\d{7,8}$/.test(val) || /^\d{11}$/.test(val)
    }, "请填写正确的电话号码(比如：0571-88888888)"), Validator.addRule("money", function(options) {
        var val = $.trim(options.element.val());
        return !val || /^\d+(.\d{1,2})?$/.test(val)
    }, "请填写正确的金额"), Validator.addRule("letter", function(options) {
        var val = $.trim(options.element.val());
        return !val || /^\w+$/.test(val)
    }, "请填写字母数字下划线"), Validator.addRule("credentials", function(options) {
        var val = $.trim(options.element.val());
        return !val || /^(\w|\(|\)){6,15}$/.test(val)
    }, "请填写字母数字下划线括号"), Validator.addRule("aliyunEmail", function(options) {
        var val = $.trim(options.element.val());
        return !val || /@aliyun\.com$/.test(val)
    }, "请填写正确的阿里云邮箱"), Validator.addRule("lastcardid", function(options) {
        var val = $.trim(options.element.val());
        return !val || /^(\d{4}|\d{3}X)$/i.test(val)
    }), Validator.addRule("multiMobile", function(options) {
        var val = $.trim(options.element.val());
        return /^(?:1\d{10}[,，])*1\d{10}$/.test(val)
    }, "请填写正确的手机号(若多个手机号，请用 “，” 隔开)"), Validator
});
"use strict";
