define("src/common/handlerbars-debug", ["common/jquery-debug", "alinw/handlebars/1.3.0/handlebars-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "model/address/data-debug"], function(require, exports, module) {
    function checkRule(targetLsit, rule) {
        var flag = !1,
            ruleList = rule.split(",");
        return util.breakEachArr(ruleList, function(val) {
            if (util.indexOfArr(targetLsit, val) !== -1) return flag = !0
        }), flag
    }

    function checkFilterDocking(action, status, isDocking, dockingStatus, values) {
        var arr = JSON.parse(values);
        if (action === arr.actionCode && status === arr.status && isDocking === arr.isDocking && dockingStatus === arr.dockingStatus) return !0
    }
    ...
    return Handlebars.compile = function(source, isElem) {
        if (isElem) {
            var elem = $(source);
            elem.length && (source = elem.html())
        } else source += "";
        var fn = compile(source);
        return fn.source = source, fn
    }, Handlebars.registerHelper("log", function(msg) {}), Handlebars.registerHelper("set", function(key, val) {
        this[key] = val
    }), Handlebars.registerHelper("wrapWord", function(str, leg) {
        str = void 0 === str ? "" : str;
        var num = str.length;
        return num > leg ? str = str.slice(0, leg) + "..." : str
        ...
        var args = Array.prototype.slice.call(arguments),
            theKey = args.shift(),
            theOptions = args.pop();
        return util.indexOfArr(args, theKey) !== -1 ? theOptions.fn(this) : theOptions.inverse(this)
    }), Handlebars.registerHelper("caseNotEnd", function(a, type, options) {
        var endStatus = "paymentorder" == type ? ["not_accepted", "execution_applied", "lawsuit_transformed", "payment_end", "dropped"] : ["conciliate", "sentenced", "jurisediction_objection", "not_accepted", "return", "dropped", "not_be_served", "cancel_apply", "unpaid_dropped"];
        return $.inArray(a, endStatus) < 0 ? options.fn(this) : options.inverse(this)
        ...
        return $el.find('[value="' + value + '"]').attr({
            selected: "selected"
        }), $el.html()
        ...
        return index !== arr.length - 1 ? options.fn(this) : options.inverse(this)
        ...
        return rule && checkRule(list.roles, rule) ? options.fn(this) : options.inverse(this)
    }), Handlebars.registerHelper("filterDocking", function(action, status, isDocking, dockingStatus, values, options) {
        return checkFilterDocking(action, status, isDocking, dockingStatus, values) ? options.fn(this) : options.inverse(this)
    }), Handlebars.registerHelper("getAreaByAreaCode", function(code) {
        return code = code.slice(0, 4) + "00", areaData[code][0]
    }), Handlebars.registerHelper("times", function(a) {
        for (var html = "", i = 1; i <= a; i++) html += "<li>" + i + "</li>";
        return html
    }), Handlebars
});
"use strict";
