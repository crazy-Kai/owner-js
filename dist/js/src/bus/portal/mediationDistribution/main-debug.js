"use strict";
define("src/bus/portal/mediationDistribution/main-debug", ["bus/global/main-debug", "common/jquery-debug", "common/delegate-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "model/filterConditions/main-debug", "model/modal/main-debug", "model/countDown/main-debug", "common/dialog-debug", "common/domUtil-debug", "common/handlerbars-debug", "model/ajax/main-debug", "model/switchFilterBtn/main-debug", "model/searchList/main-debug", "common/statusMap-debug"], function(require, exports, module) {
    function doSearch() {
    ...
    function editMediator(e, person, city, param) {
        ...
        Dialog.showTemplate("#chose-mediator", $.extend(childParam, {
            city: city.data,
            person: person.data
        }), {
            width: 800,
            ...
            events: {
                'change [name="city"]': function(e) {
                    ...
                    new Ajax({
                        ...
                        me.$("#mediator-list").html(mediatorList({
                            person: person.data
                        }))
                    }).submit()
                },
                "click .JS-trigger-submit": function(e) {
                    ...
                    }).submit() : Modal.alert(0, "请选择调解员")
                },
                "click #mediator-list tr": function(e) {
                    var tr = $(e.target).closest("tr");
                    tr.find('input[type="radio"]').prop("checked", !0)
                }
            }
        })
    }
    ...
    new SwitchFilterBtn({
        ...
        new Ajax({
            ...
            $.each(city.data, function() {
                "杭州市" === this.cityName && (cityId = this.securityId)
            }), new Ajax({
                request: "/hephaistos/mediatorDistributionRpc/MediatorListForDistribution.json",
                param: $.extend({}, param, {
                    cityId: cityId
    ...
    var mediatorList = Handlerbars.compile("#mediator-seed", !0);
    new Ajax({
        ...
        events: {
            'click [data-role="mediationEnd"]': function(e) {
                var me = this;
                e.preventDefault(), me.set("request", "/account/mySuitRpc/mediationEnd.json"), me.set("param", $(e.target).data("param")), Modal.confirm("提示", "请确认是否提前结束调解，递交到法院立案审核，立案后还可以进行调解", function() {
                    me.submit()
                })
            },
            'click [data-role="mediationDelay"]': function(e) {
                ...
                for (var param = $(e.target).data("param"), list = param.list = [], i = 0; i < param.mediateDayDelayMax - ~~param.mediateDayDelay;) list[i] = ++i;
