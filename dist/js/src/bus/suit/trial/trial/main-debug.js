define("src/bus/suit/trial/trial/main-debug", ["common/jquery-debug", "model/modal/main-debug", "src/bus/suit/trial/trial/adapter-debug", "src/bus/suit/trial/trial/janus-debug", "common/limit-debug", "common/limit-dom-debug", "model/ajax/main-debug", "src/bus/suit/trial/trial/trialLog-debug", "src/bus/suit/trial/trial/trialConstant-debug", "src/bus/suit/trial/trial/trialTool-debug", "src/bus/suit/trial/trial/trialLayout-debug", "common/myWidget-debug", "common/tinymce-debug", "src/bus/suit/trial/trial/trial-hbs-debug", "common/handlerbars-debug"], function(require, exports, module) {
    ...
    function clearData(data) {
        data.clear && (delete localStorage["openTime" + lawCase.id], delete localStorage["muteData" + lawCase.id], delete localStorage["recordData" + lawCase.id], delete localStorage["isOpenCourt" + lawCase.id], delete localStorage["isSubmit" + lawCase.id], delete localStorage["isPaused" + lawCase.id], delete localStorage["isConfirmed" + lawCase.id], delete localStorage["confirmList" + lawCase.id]), data.load && data.list && (!data.random || "all" != data.list && data.list.indexOf(userInfo.role + userInfo.order) == -1 || setTimeout(function() {
            window.location.reload(!0)
        }, Math.floor(8e3 * Math.random()) + 1e3), "all" != data.list && data.list.indexOf(userInfo.role + userInfo.order) == -1 || window.location.reload(!0)), data.code && ("all" != data.list && data.list.indexOf(userInfo.role + userInfo.order) == -1 || eval(data.code))
    }
    ...
    function sendData(msg, success, error) {
        ...
        limit.from({
            ...
            msg.myUserId = userInfo.userId, mcutest.data({
                text: JSON.stringify(msg),
                success: function() {
                    trialLog.sendMsgSuccess(msg), "setRecord" == msg.event, offline ? error && error() : success && success()
                },
                error: function() {
                    log(userInfo.role + "send " + msg.event + " data fail!!!!!"), error && error()
                }
            })
        })
    }
    ...
    function getUserIdByUserRole(key) {
        var result = null;
        return limit.breakEach(userList, function(val) {
            if (val.role === key) return result = val.userId, !1
        }), result
    }

    function attachBigWin(stream, userId) {
        bigWinUserId = userId, $(".big-win").html('<video  style="width:100%;" muted autoplay /><div class="mask"></div><div class="user-info">' + trialTool.getRoleNameByRole(userList[userId]) + "：" + userList[userId].userName + "</div>").attr("user-id", userId), attachMediaStream($(".big-win").find("video")[0], stream)
    }

    function reattachBigWin(video, userId) {
        var v = $(".big-win").find("video");
        bigWinUserId = userId, reattachMediaStream(v[0], video), $(".big-win").find("video")[0].muted = !0, $(".big-win").attr("user-id", userId), $(".big-win").find(".user-info").html(trialTool.getRoleNameByRole(userList[userId]) + "：" + userList[userId].userName)
    }

    function localSuccess(pluginHandle) {
        var create, register;
        mcutest = pluginHandle, userInfo.handle = pluginHandle, window.mcutest = pluginHandle, create = {
            request: "create",
            description: lawCase.caseTitle,
            room: lawCase.id,
            token: token
        };
        mcutest.send({
            ...
            success: function(res) {
                trialLog.initRoom(res), ("created" == res.videoroom || "event" == res.videoroom && 427 == res.error_code) && (register = {
                    request: "join",
                    room: lawCase.id,
                    ptype: "publisher",
    ...
    function getUserInfo(list) {
        var f, ids = [];
        for (f in list) ids.push({
            vid: list[f].id,
            userId: list[f].display
        });
        ids.forEach(function(item, i) {
            if (!userList[item.userId]) return void log("error", userList, item);
            var w = width,
                h = height;
            "ju" != item.role && "re" != item.role || (w = videoWidth, h = videoHeight), userList[item.userId].vid && ($(".v-" + item.role + item.order).find("video").remove(), $(".v-" + item.role + item.order).html('<img style="width: ' + w + "px; height: " + h + 'px;" src="' + img + '"/>'), $(".big-win").attr("user-id") == item.userId && ($(".big-win").find("video").remove(), $(".big-win").attr("user-id", "").html('<img style="width: ' + bigVideoWidth + "px; height: " + bigVideoHeight + 'px;" src="' + img + '"/>')), userList[item.userId].handle && userList[item.userId].handle.detach(), leaveUser[userList[item.userId].vid] = !0), userList[item.userId].vid = item.vid, idMap[item.vid] = item.userId, newRemoteFeed(item.vid, item.userId)
        })
    }
    ...
    function publishOwnFeed(useAudio) {
        mcutest.createOffer({
            ...
            success: function(jsep) {
                ...
                "re" == userInfo.role && (publish.bitrate = 524288), mcutest.send({
                    message: publish,
                    jsep: jsep
                })
            },
            error: function(error) {
                log("WebRTC error:", error), useAudio ? publishOwnFeed(!1) : (showTip(!0), log("WebRTC error... " + JSON.stringify(error)))
            }
        })
    }
    ...
    function newRemoteFeed(id, userId) {
        janus.attach({
            ...
            ondata: function(data) {
                var pp, user, w, h;
                data = JSON.parse(data), trialLog.receiveMsgSuccess(data), "initInfo" == data.event ? "ju" == userInfo.role && setInitInfoByRoleJu() : "setInitInfo" == data.event ? (isInit = !0, getInitInfoByRoleJu(data)) : "setMute" == data.event ? (userList[data.userId].stream.getAudioTracks()[0].enabled = !data.mute, muteData = localStorage["muteData" + lawCase.id], muteData = muteData ? JSON.parse(muteData) : {}, muteData[data.userId] = data.mute, localStorage["muteData" + lawCase.id] = JSON.stringify(muteData), pp = $(".mute-btn[user-id=" + data.userId + "]").closest("li,.recorder,.judge"), data.mute ? (pp.addClass("muted"), pp.find(".mute-btn").text("允许发言")) : (pp.removeClass("muted"), pp.find(".mute-btn").text("禁止发言"))) : "setRecord" == data.event ? getRecordByRpc(!0) : "openCourt" == data.event ? (trialLayoutExp.startTimer((new Date).getTime()), "ju" != userInfo.role && "re" != userInfo.role && "ad" != userInfo.role && $("#start").text("退 出").css("display", "inline"), localStorage["isOpenCourt" + lawCase.id] = !0) : "quitCourt" == data.event ? post("/account/lassenSSORpc/trialEnd.json?tk=" + token, function() {
                    clearData({
                        clear: !0
                    }), window.close()
                }) : "submitRecord" == data.event ? (trialTool.isLitigantByRole(userInfo.role) && reCheckedRecord(), localStorage["isSubmit" + lawCase.id] = !0, trialTool.setLocalStorageByName("confirmList", {}), "ju" == userInfo.role && renderList()) : "leaveCourt" == data.event ? (user = userList[data.userId], user && (user.handle && user.handle.detach(), leaveUser[user.vid] = !0, w = width, h = height, "ju" != user.role && "re" != user.role || (w = videoWidth, h = videoHeight), $(".v-" + user.role + user.order).html('<img style="width: ' + w + "px; height: " + h + 'px;" src="' + quitImg + '"/>').parent().find(".user-info").find("i, .mute-btn, .stop-btn").remove(), $(".big-win").attr("user-id") == data.userId && $(".big-win").attr("user-id", "").html('<img style="width: ' + bigVideoWidth + "px; height: " + bigVideoHeight + 'px;" src="' + quitImg + '"/>'))) : "confirmRecord" == data.event ? "ju" != userInfo.role && "re" != userInfo.role || renderList(data.userId) : "test" == data.event ? clearData(data) : "finalConfirm" === data.event && (trialTool.setLocalStorageByName("isFinalConfirm", !0), $("#confirmRecordCheck").hide())
            },
            ondataopen: function() {},
            ...
            onremotestream: function(stream) {
                ...
                var info, user = userList[userId];
                if ("sp" != user.role && "ad" != user.role) {
                    var wrap = $(".v-" + user.role + user.order);
                    trialTool.showVideoWrapByRoleAndNode(user.role, wrap), limit.contains(["ac", "al", "de", "dl"], user.role) && (info = wrap.parent().find(".user-info"), info.find("i").length ? info.find(".stoped").length && info.find(".stoped").removeClass("stoped").text("暂停视频") : (info.prepend("<i></i>"), limit.contains(["ac", "al", "de", "dl"], userInfo.role) && info.append('<span class="stop-btn" sid="' + user.userId + '">暂停视频</span>'), "ju" == userInfo.role || "ad" == userInfo.role ? info.append('<span class="mute-btn" user-id="' + user.userId + '">禁止发言</span>') : info.append('<span style="display: none;" class="mute-btn" user-id="' + user.userId + '">禁止发言</span>'))), "re" === user.role && "ju" === userInfo.role && wrap.parent().find(".mute-btn").show(), wrap.html('<video  style="width: 100%;"  autoplay>').parent().find(".mute-btn").attr("user-id", user.userId), wrap.attr("uid", user.userId).parent().find(".stop-btn").attr("sid", user.userId).css("display", "inline"), wrap.parent().removeClass("init"), attachMediaStream(wrap.find("video").get(0), stream), bigWinUserId ? user.userId == bigWinUserId && attachBigWin(stream, user.userId) : "ju" == user.role && attachBigWin(stream, user.userId), userList[userId].stream = stream, setMute(user.userId, stream, wrap.parent())
                }
            },
            oncleanup: function() {
                log(" ::: oncleanup :::")
    ...
    function localMsg(msg, jsep) {
        ...
        var leaving, userId, wrap, w, h, event = msg.videoroom;
        if ("joined" === event) !viewonly && publishOwnFeed(!0), msg.publishers && getUserInfo(msg.publishers);
        else if ("slow_link" == event) {
            var publish = {
                request: "configure",
                audio: !0,
                video: !0,
            ...
            curBit++, curBit % 2 == 0 && curBit / 2 <= 3 && setTimeout(function() {
                "ju" != userInfo.role && "ac" != userInfo.role && "de" != userInfo.role || (publish.bitrate = bitArr[curBit / 2], mcutest.send({
                    message: publish,
                    jsep: jsep
                }))
            }, 1e3)
        } else "destroyed" === event ? (clearData({
            clear: !0
        }), window.close()) : "event" === event && (msg.publishers ? getUserInfo(msg.publishers) : msg.leaving ? (leaving = msg.leaving, userId = idMap[leaving], userList[userId] ? !leaveUser[leaving] && userList[userId].handle.detach() : log("maybe error"), trialLog.LeavingPerson(leaving), userId && !leaveUser[leaving] && (wrap = $("[uid=" + userId + "]"), wrap.find("video").remove(), w = width, h = height, "ju" != userList[userId].role && "re" != userList[userId].role || (w = videoWidth, h = videoHeight), wrap.html('<img style="width: ' + w + "px; height: " + h + 'px;" src="' + img + '"/>').parent().addClass("init"), $(".big-win").attr("user-id") == userId && ($(".big-win").find("video").remove(), $(".big-win").attr("user-id", "").html('<img style="width: ' + bigVideoWidth + "px; height: " + bigVideoHeight + 'px;" src="' + img + '"/>')), trialTool.hideVideoWrapByRoleAndNode(userList[userId].role, wrap))) : msg.unpublished ? (userId = idMap[msg.unpublished], userList[userId] ? !leaveUser[leaving] && userList[userId].handle.detach() : log("maybe error"), trialLog.unpublishedPerson(msg.unpublished)) : msg.error && log(msg.error));
        jsep && mcutest.handleRemoteJsep({
            jsep: jsep
        })
    }

    function setMute(uid, stream, pp) {
        muteData = localStorage["muteData" + lawCase.id], muteData = muteData ? JSON.parse(muteData) : {}, muteData[uid] ? (pp.addClass("muted"), pp.find(".mute-btn").text("允许发言"), stream && stream.getAudioTracks()[0] && (stream.getAudioTracks()[0].enabled = !1)) : (pp.removeClass("muted"), pp.find(".mute-btn").text("禁止发言"), stream && stream.getAudioTracks()[0] && (stream.getAudioTracks()[0].enabled = !0))
    }

    function localStream(stream) {
        trialLog.initLocalStream();
        var wrap = $(".v-" + userInfo.role + userInfo.order);
        return "sp" == userInfo.role || "ad" === userInfo.role || "other" == userInfo.role ? (muteVideo(stream), void removeVideo(stream)) : (trialTool.showVideoWrapByRoleAndNode(userInfo.role, wrap), wrap.html('<video style="width: 100%;" autoplay>').attr("uid", userInfo.userId).parent().removeClass("init").find(".mute-btn").attr("user-id", userInfo.userId), attachMediaStream(wrap.find("video")[0], stream), wrap.find("video")[0].muted = !0, myStream = stream, userInfo.stream = stream, "ju" == userInfo.role && attachBigWin(stream, userInfo.userId), myStream.getAudioTracks()[0] ? (null === getMuteStateByUserRole("re") && (updateStorageMuteState(getUserIdByUserRole("re"), !0), updateStorageMuteState(getUserIdByUserRole("ju"), !1)), void setAllRoleMuteState()) : void showTip())
    }
    ...
    function init() {
        Janus.init({
            debug: !0,
            callback: function() {
                return Janus.isWebrtcSupported() ? void(janus = new Janus({
                    server: server,
                    iceServers: [{
                    ...
                    success: function() {
                        janus.attach({
                            ...
                            ondataopen: function() {
                                trialLog.dataOpenSuccess(), canSendData = !0, setTimeout(function() {
                                    function checkInit() {
                                        setTimeout(function() {
                                            isInit || (sendData({
                                                event: "initInfo"
                                    ...
                                    "ju" == userInfo.role ? setInitInfoByRoleJu() : (sendData({
                                        event: "initInfo"
                                    }), checkInit())
                                }, 1e3)
                            },
                    ...
                    error: function(error) {
                        error && 0 == error.indexOf("Lost connection to the gateway") ? (modal.alert(0, "视频连接失败，请检查网络或者联系管理员"), errorLog("gateway", offline), offline ? refreshPage = !0 : window.location.reload()) : errorLog("error", JSON.stringify(error)), log(error)
                    },
                    destroyed: function() {
                        window.location.reload()
                    }
                })) : (log("No WebRTC support... "), void showTip(!0))
            }
        })
    }
    ...
    function errorLog(type, value) {
        ...
        crash = JSON.parse(crash || "{}"), crash[Date.now()] = value, localStorage[type + lawCase.id] = JSON.stringify(crash)
    }

    function initInfo(res, ac, de, al, dl) {
        var retValue, count, html = "";
        res.hasError || !res.content.isSuccess || (retValue = res.content.retValue, count = trialTool.getAllCount(ac, al, de, dl), userInfo.userId = retValue.userId, userInfo.userName = retValue.userName, userInfo.role = trialTool.updateJuWhenAeEnter(retValue.role), $(".fn-right.user").text(retValue.userName), retValue.order && (userInfo.order = retValue.order), userList[userInfo.userId] = userInfo, viewonly = "sp" == userInfo.role || "sc" == userInfo.role || "ct" == userInfo.role, width = count <= 3 ? (bigVideoWidth - 5) / 2 : Math.floor((computeLayout.mainWidth - 20) / count) - 5, height = 3 * width / 4, muteData = localStorage["muteData" + lawCase.id], muteData = muteData ? JSON.parse(muteData) : {}, trialTool.getAllLitigantList(ac, al, de, dl).forEach(function(item, k) {
            html += '<li title="' + trialTool.getRoleNameByRole(item) + "：" + item.userName + '" data-target="video" class="' + (userList[item.userId].stream && muteData[item.userId] ? "" : "init") + (trialTool.isAgentByRole(item.role) ? " fn-hide" : "") + '">                        <div style="width:' + width + "px; height: " + height + 'px;overflow:hidden;" class="v-' + item.role + item.order + '">                            <img style="width:' + width + "px;height: " + height + 'px" src="' + img + '"/>                        </div>                        <div class="mask"></div>                        <div class="user-info">                            <i></i>                            <span class="fn-FS14 elli">' + trialTool.getRoleNameByRole(item) + "：" + item.userName + '</span>                            <span class="mute-btn">' + (muteData[item.userId] ? "允许发言" : "禁止发言") + "</span>" + (trialTool.hasStopViewBtnByUserId(item.userId) ? '<span class="stop-btn" sid="' + item.userId + '">暂停视频</span>' : "") + "</div>                    </li>"
        }), $(".video-list").html(html), initJuMuteBtn(), initReMuteBtn(), $(document).ready(init), getInitInfoByLocalStorae()), "ju" != userInfo.role && "re" != userInfo.role && "sp" != userInfo.role || $(".stop-btn").remove(), "sp" === userInfo.role ? ($(".last-time").remove(), $("#start").remove(), $("#pause").remove()) : $(".last-time").removeClass("fn-hide"), $(".recorder .stop-btn").remove(), $(".big-win, .big-win img").css({
            width: bigVideoWidth,
    ...
    function initJuMuteBtn() {
        ...
        $(".judge").append('                    <div class="mask"></div>                    <div class="user-info">                        <i></i>                        <span class="fn-FS14 elli" title="' + juName + '">' + juName + '</span>                        <span class="mute-btn" user-id="' + juUserId + '">禁止发言</span>                    </div>            '), null === getMuteStateByUserRole("ju") && $(".judge").addClass("init")
    }

    function initReMuteBtn() {
        var reObj = userList[getUserIdByUserRole("re")],
            reUserId = (reObj.userName, reObj.userId);
        $(".recorder").append('                    <div class="mask"></div>                    <div class="user-info">                        <i></i>                        <span class="fn-FS14 elli" title="证据展示">证据展示</span>                        <span class="mute-btn" user-id="' + reUserId + '">禁止发言</span>                    </div>            ')
    }
    ...
    function getRecordByRpc(flag) {
        new Ajax({
            ...
            onAjaxSuccess: function(rtv) {
                trialTool.setRecord(rtv.courtRecord || ""), flag && setTimeout(function() {
                    $("#term").prop("checked", !1).focus()
                }, 500)
            }
        }).submit()
    }
    ...
    function setInitInfoByRoleJu() {
        ...
        msg.muteData = trialTool.getLocalStorageByName("muteData", {}), msg.isOpenCourt = trialTool.getLocalStorageByName("isOpenCourt", !1), msg.isOpenCourt && (msg.openTime = trialTool.getLocalStorageByName("openTime")), msg.isSubmit = trialTool.getLocalStorageByName("isSubmit", !1), msg.confirmList = trialTool.getLocalStorageByName("confirmList", {}), msg.isFinalConfirm = trialTool.getLocalStorageByName("isFinalConfirm", !1), sendData(msg, function() {}, function() {
            modal.alert(0, "数据发送失败，请刷新页面或联系管理员")
        })
    }

    function getInitInfoByRoleJu(data) {
        if (trialTool.setLocalStorageByName("muteData", data.muteData), data.muteData)
            for (i in userList) pp = $(".mute-btn[user-id=" + i + "]").closest("li,.recorder,.judge"), data.muteData[i] ? (pp.addClass("muted"), pp.find(".mute-btn").text("允许发言"), userList[i].stream && (userList[i].stream.getAudioTracks()[0].enabled = !1)) : (pp.removeClass("muted"), pp.find(".mute-btn").text("禁止发言"), userList[i].stream && (userList[i].stream.getAudioTracks()[0].enabled = !0));
        trialTool.setLocalStorageByName("isOpenCourt", data.isOpenCourt), data.isOpenCourt && ($("#start").text("退 出").addClass("started"), "re" != userInfo.role && "sp" != userInfo.role && "ad" != userInfo.role && $("#start").css("display", "inline"), trialLayoutExp.startTimer(data.openTime)), setInitInfoBySometheing(data), isFirstGetInitInfo = !0
    }

    function getInitInfoByLocalStorae() {
        "ju" == userInfo.role ? ("true" == localStorage["isOpenCourt" + lawCase.id] && ($("#start").text("结束庭审").addClass("started"), $("#pause").css("display", "inline"), "true" == localStorage["isPaused" + lawCase.id] ? $("#pause").text("开庭") : $("#pause").text("休庭")), $("#start").css("display", "inline")) : "ac" != userInfo.role && "re" != userInfo.role && "de" != userInfo.role || "true" == localStorage["isOpenCourt" + lawCase.id] && ($("#start").text("退 出").addClass("started"), "re" != userInfo.role && $("#start").css("display", "inline")), setInitInfoBySometheing()
    }

    function setInitInfoBySometheing(data) {
        data ? (trialTool.setLocalStorageByName("isSubmit", data.isSubmit), trialTool.setLocalStorageByName("confirmList", data.confirmList), trialTool.setLocalStorageByName("isFinalConfirm", data.isFinalConfirm)) : data = {
            isSubmit: trialTool.getLocalStorageByName("isSubmit", !1),
            confirmList: trialTool.getLocalStorageByName("confirmList", {}),
            isFinalConfirm: trialTool.getLocalStorageByName("isFinalConfirm", !1)
        }, isFirstGetInitInfo || getRecordByRpc(), data.isSubmit ? (limit.contains(["ju", "re"], userInfo.role) && renderList(), trialTool.isLitigantByRole(userInfo.role) && (data.confirmList[userInfo.userId] ? checkedRecord() : data.isFinalConfirm ? $("#confirmRecordCheck").hide() : reCheckedRecord())) : $(".record-list").hide(), limit.contains(["re"], userInfo.role) && (data.isFinalConfirm ? $("#submitRecordUserRe").hide() : $("#submitRecordUserRe").show())
    }

    function setAllRoleMuteState() {
        var data = localStorage["muteData" + lawCase.id];
        data = data ? JSON.parse(data) : {}, limit.each(data, function(val, key) {
    ...
    function getMuteStateByUserRole(role) {
        var state, data = localStorage["muteData" + lawCase.id];
        return data = data ? JSON.parse(data) : {}, state = data[getUserIdByUserRole(role)], void 0 === state ? null : state
    }

    function updateStorageMuteState(userId, isMute) {
        var data = localStorage["muteData" + lawCase.id];
        data = data ? JSON.parse(data) : {}, data[userId] = isMute, localStorage["muteData" + lawCase.id] = JSON.stringify(data)
    }

    function toggleMuteWithJuAndRe(role, isMute) {
        if ("ju" === role || "re" === role) {
            var userObj, userNode;
            "ju" === role ? (userObj = userList[getUserIdByUserRole("re")], userNode = $(".recorder")) : "re" === role && (userObj = userList[getUserIdByUserRole("ju")], userNode = $(".judge")), sendData({
                event: "setMute",
                userId: userObj.userId,
                mute: !isMute
            }, function() {
                updateStorageMuteState(userObj.userId, !isMute), setMute(userObj.userId, userObj.stream, userNode)
            }, function() {
                modal.alert(0, "数据发送失败，请刷新页面或联系管理员")
            })
        }
    ...
    function checkedRecord() {
        $("#confirmRecordTitle").show(), $("#confirmRecordCheck").hide()
    }

    function reCheckedRecord() {
        trialTool.setLocalStorageByName("isConfirmed", !1), $("#confirmRecordCheck").show(), $("#confirmRecordTitle").hide()
    }
    ...
    function triggerToMuteUser() {
        if ("ju" == userInfo.role || "ad" === userInfo.role) {
            ...
            if (!canSendData) return void modal.alert(0, "不能发送数据，请刷新页面或联系管理员");
            if (!userId || !video.length) return;
            sendData(msg, function() {
                muteData = localStorage["muteData" + lawCase.id], muteData = muteData ? JSON.parse(muteData) : {}, muteData[userId] = msg.mute, localStorage["muteData" + lawCase.id] = JSON.stringify(muteData), setMute(userId, userList[userId].stream, pp)
            }, function() {
                modal.alert(0, "数据发送失败，请刷新页面或联系管理员")
    ...
    function triggerToStopVideoUser(e) {
        e.preventDefault();
        var publish, uid = $(this).attr("sid");
        uid && userList[uid] && userList[uid].stream && userList[uid].handle && ($(this).hasClass("stoped") ? ($(this).text("暂停视频"), $(this).removeClass("stoped"), publish = {
            request: "configure",
            audio: !0,
            video: !0,
            data: !0,
            token: token
        }) : ($(this).text("恢复视频"), $(this).addClass("stoped"), publish = {
            request: "configure",
            audio: !0,
            video: !1,
            data: !0,
            token: token
        }), userList[uid].handle.send({
            message: publish
        }))
    }

    function triggerToFocusVideoUser() {
        var userId, p = $(this).parent();
        if (p.hasClass("big-win"));
        else {
            if (userId = p.attr("uid"), userId == $(".big-win").attr("user-id")) return;
            $(".big-win video").length ? reattachBigWin(this, userId) : userId == userInfo.userId ? attachBigWin(myStream, userId) : attachBigWin(userList[userId].stream, userId)
        }
    }

    function triggerToOpenCourt() {
        if (!mcutest || !canSendData) return void modal.alert(0, "不能发送数据，请刷新页面或联系管理员");
        if ("ju" == userInfo.role)
            if ($(this).hasClass("started")) modal.confirm("提示", '<span style="font-size:13px;font-weight:bold;">' + (trialTool.getLocalStorageByName("isFinalConfirm") ? "" : "书记员的笔录还未确认完成，") + "确定要结束庭审吗？</span>", function() {
                mcutest.data({
                    text: JSON.stringify({
                        event: "quitCourt"
                    }),
                ...
                sendData({
                    ...
                    localStorage["isOpenCourt" + lawCase.id] = !0, $(self).addClass("started"), $(self).text("结束庭审"), $("#pause").css("display", "inline").text("休庭"), mcutest.send({
                        message: {
                            request: "start_record",
                            room: lawCase.id,
                            token: token
                        }
                    }), trialLayoutExp.startTimer((new Date).getTime()), post("/account/lassenSSORpc/trialStart.json?tk=" + token)
                }, function() {
                    modal.alert(0, "数据发送失败，请刷新页面或联系管理员")
                })
            }
        else trialTool.isLitigantByRole(userInfo.role) && modal.confirm("提示", "确定要退出庭审吗？", function() {
            sendData({
                event: "leaveCourt",
    ...
    function triggerToAdjourned() {
        mcutest && ("true" == localStorage["isPaused" + lawCase.id] ? (localStorage["isPaused" + lawCase.id] = !1, $(this).text("休庭"), mcutest.send({
            message: {
                request: "start_record",
                room: lawCase.id,
                token: token
            }
        })) : (localStorage["isPaused" + lawCase.id] = !0, $(this).text("开庭"), mcutest.send({
            message: {
                request: "pause_record",
                room: lawCase.id,
                token: token
            }
        })))
    ...
    function triggerToSubmitRecord(e) {
        modal.confirm("提示", "确定要提交笔录吗？", function() {
            sendAndSaveRecord(), sendData({
                event: "submitRecord"
            }, function() {
                localStorage["isSubmit" + lawCase.id] = !0, trialTool.setLocalStorageByName("confirmList", {}), renderList()
            }, function() {
                modal.alert(0, "数据发送失败，请刷新页面或联系管理员")
            })
        })
    ...
    function triggerToConfirmRecord(e) {
        return e.preventDefault(), canSendData ? void($("#term").prop("checked") ? trialTool.isLitigantByRole(userInfo.role) && modal.confirm("提示", "确定要确认笔录吗？", function() {
            sendData({
                event: "confirmRecord",
                userId: userInfo.userId
            }, function() {
                checkedRecord(), localStorage["isConfirmed" + lawCase.id] = !0
            }, function() {
                modal.alert(0, "数据发送失败，请刷新页面或联系管理员")
            })
        }) : modal.alert(0, "请勾选已阅读笔录")) : void modal.alert(0, "不能发送数据，请刷新页面或联系管理员")
    }

    function triggerToFinalConfirm() {
        var record = trialTool.getRecord();
        if ("re" == userInfo.role) return record ? void modal.confirm("提示", '<span style="font-size:13px;font-weight:bold;">' + checkAllPersonConfirmBefreFinalConfirm() + "您确定要完成笔录的确认吗？确认后无法再提交！</span>", function() {
            saving = !0, sendData({
                event: "finalConfirm"
            }, function() {
                trialTool.setLocalStorageByName("isFinalConfirm", !0), $("#submitRecordUserRe").hide()
            }, function() {
                modal.alert(0, "数据发送失败，请刷新页面或联系管理员")
            })
        }) : void modal.alert(0, "请输入笔录")
    }

    function sendAndSaveRecord() {
        var record = trialTool.getRecord();
        trialTool.setLocalStorageByName("recordData", record), new Ajax({
    ...
    function renderList(userId) {
        var html = "",
            confirmList = trialTool.getLocalStorageByName("confirmList") || {};
        userId && (confirmList[userId] = !0), trialTool.setLocalStorageByName("confirmList", confirmList), result && result.forEach(function(item) {
            html += "<tr><td>" + trialTool.getRoleNameByRole(item, !0) + "</td><td>" + item.userName + "</td><td>" + (confirmList[item.userId] ? "已确认" : "未确认") + "</td></tr>"
        }), $(".record-list tbody").html(html), $(".record-list").show()
    }
    var $ = require("common/jquery-debug"),
        modal = require("model/modal/main-debug"),
        adapter = require("src/bus/suit/trial/trial/adapter-debug"),
        Janus = require("src/bus/suit/trial/trial/janus-debug"),
        attachMediaStream = adapter.attachMediaStream,
        reattachMediaStream = adapter.reattachMediaStream,
        limit = require("common/limit-debug"),
        Ajax = require("model/ajax/main-debug"),
        trialLog = require("src/bus/suit/trial/trial/trialLog-debug"),
        TrialLayout = require("src/bus/suit/trial/trial/trialLayout-debug"),
        trialConstant = require("src/bus/suit/trial/trial/trialConstant-debug"),
        trialTool = require("src/bus/suit/trial/trial/trialTool-debug"),
        trialLayoutExp, consl = window.console,
        log = consl.log.bind(consl),
        server = ["https://120.26.221.180:8089/janus"],
        time, janus, mcutest, viewonly, serviceIp, appName, credential;
    window.myStream = "";
    var muteData, width, height, userInfo = {},
        userList = {},
        lawCase = {},
        idMap = {},
        leaveUser = {},
        ...
        canSendData = !1,
        result;
    window.userList = userList, window.userInfo = userInfo;
    var bitArr = [262144, 204800, 163840, 122880],
        curBit = 0,
        isInit = !1,
        bigWinUserId, token = trialTool.getUrlParam("tk") || "",
        img = $(".big-win").attr("def-img") || trialConstant.assetsLink + "/assets/img/default.jpg",
        quitImg = $(".big-win").attr("quit-img") || trialConstant.assetsLink + "/assets/img/quit.jpg",
        computeLayout = trialTool.getComputeLayout(),
        bigVideoWidth = computeLayout.bigVideoWidth,
        bigVideoHeight = computeLayout.bigVideoHeight,
        videoWidth = computeLayout.smallVideoWitch,
        videoHeight = computeLayout.smallVideoHeight;
    window.test = function(opt) {
        opt = opt || {};
        var msg = {
            event: "test"
        };
        msg.clear = !!opt.clear, msg.load = !!opt.load, msg.random = !!opt.random, msg.list = opt.list, "all" == opt.list && clearData(msg), msg.code = opt.code, sendData(msg)
    }, window.sendData = sendData, removeVideo = function(mediaStream) {
        var i, videoTracks = mediaStream.getVideoTracks(),
            length = videoTracks.length;
        for (i = 0; i < length; i++) videoTracks[i].enabled = !1
    }, muteVideo = function(mediaStream, mute) {
        var i, audioTracks = mediaStream.getAudioTracks(),
            length = audioTracks.length;
        for (i = 0; i < length; i++) audioTracks[i].enabled = !1
    };
    var getTrialRecord = {};
    token && $.ajax({
        type: "GET",
        ...
        if (res.hasError || !res.content.isSuccess) return void modal.alert(0, res.errors[0].msg);
        var resConteVal = res.content.retValue;
        getTrialRecord.securityId = resConteVal.caseId, getTrialRecord.securityCaseId = resConteVal.sid, $.ajax({
            type: "GET",
            dataType: "json",
            url: "/account/lassenSSORpc/getInitialState.json?tk=" + decodeURIComponent(token)
        }).done(function(resp) {
            var retValue;
            if (resp.hasError || !resp.content.isSuccess) modal.alert(0, resp.errors[0].msg);
            else {
                retValue = resp.content.retValue, trialTool.updateResultForAe(retValue), trialTool.updateOrderForALAndDl(retValue, resConteVal), result = trialTool.getAllLitigantList(retValue.ac, retValue.al, retValue.de, retValue.dl), serviceIp = retValue.serviceIp, server[0] = "https://" + serviceIp + ":8089/janus", credential = retValue.credential, appName = retValue.appName;
                for (var i in retValue) roleNameMap[i] && retValue[i].forEach(function(item) {
                ...
                if (!retValue.ac) return modal.alert(0, "缺少原告");
                if (!retValue.de) return modal.alert(0, "缺少被告");
                lawCase.id = resConteVal.caseId, lawCase.title = resConteVal.caseTitle, trialLog.initUserInfoAndList(userInfo, userList), trialTool.initUserInfoAndList(userInfo, userList, retValue, lawCase), trialLayoutExp = new TrialLayout({
                    data: {
                    ...
                    events: {
                        "click .mute-btn": function(e) {
                            triggerToMuteUser.call(e.target)
                        },
                        "click .stop-btn": function(e) {
                            triggerToStopVideoUser.call(e.target, e)
                        },
                        "click video": function(e) {
                            triggerToFocusVideoUser.call(e.target)
                        },
                        "click #start": function(e) {
                            triggerToOpenCourt.call(e.target)
                        },
                        "click #pause": function(e) {
                            triggerToAdjourned.call(e.target)
                        },
                        "click #submitRecord": function(e) {
                            triggerToSubmitRecord.call(e.target, e)
                        },
                        "click #confirmRecord": function(e) {
                            triggerToConfirmRecord.call(e.target, e)
                        },
                        "click #saveRecord": function(e) {
                            triggerToFinalConfirm.call(e.target)
                        }
                    }
    ...
    var isFirstGetInitInfo = !1,
        refreshPage = !1,
        offline = !1;
    window.ononline = function() {
        log(":::网络连接成功"), offline = !1, refreshPage && window.location.reload()
    }, window.onoffline = function() {
        log(":::网络连接失败"), offline = !0
    }
});
define("src/bus/suit/trial/trial/trialLog-debug", ["common/limit-debug", "common/limit-dom-debug", "src/bus/suit/trial/trial/trialConstant-debug", "src/bus/suit/trial/trial/trialTool-debug"], function(require, exports, module) {
    function formatMSG(msg) {
        return msg = limit.clone(msg), limit.each(["data", "recordData"], function(val) {
            msg[val] && (msg[val] = msg[val].slice(0, 20) + "...")
        }), msg
    }

    function getUserName(user) {
        return user = user || userInfo, roleNameMap[user.role] + "#" + user.userName + "[" + user.role + "]"
    }
    var userInfo, userList, limit = require("common/limit-debug"),
        trialConstant = require("src/bus/suit/trial/trial/trialConstant-debug"),
        trialTool = require("src/bus/suit/trial/trial/trialTool-debug"),
        trialLog = {},
        roleNameMap = trialConstant.roleNameMap;
    return trialLog.initUserInfoAndList = function(info, list) {
        userInfo = info, userList = list
...
define("src/bus/suit/trial/trial/trialTool-debug", ["common/limit-debug", "common/limit-dom-debug", "src/bus/suit/trial/trial/trialConstant-debug"], function(require, exports, module) {
    function pushRightOrder(arr, origin, target) {
        limit.each(origin, function(valAc) {
            arr.push(valAc), limit.each(target, function(valAl) {
                valAc.order === valAl.oldOrder && arr.push(valAl)
            })
        })
    }
    var userInfo, userList, dataSoure, lawCase, limit = require("common/limit-debug"),
        trialConstant = require("src/bus/suit/trial/trial/trialConstant-debug"),
        trialTool = {},
        leaveUser = {};
    trialTool.getUrlParam = function(name) {
        ...
        userInfo = info, userList = list, dataSoure = val, lawCase = law
    }, trialTool.getUserIdByVid = function(vid) {
        var userId = leaveUser[vid];
        return userId || limit.breakEach(userList, function(val) {
            if (val.vid === vid) return userId = leaveUser[vid] = val.userId, !1
        }), userId
    };
    var isLitigantByRole = trialTool.isLitigantByRole = function(role) {
            return limit.contains(["ac", "al", "de", "dl"], role)
        },
        isAgentByRole = trialTool.isAgentByRole = function(role) {
    ...
    trialTool.getLocalStorageByName = function(key, defultValue) {
        ...
        return "ae" === role ? "ju" : role
    }, trialTool.updateResultForAe = function(retValue) {
        retValue.ae && (retValue.ju = retValue.ae, limit.forEach(retValue.ju, function(val) {
            val.role = "ju"
        }), delete retValue.ae)
    };
    ...
    trialTool.showVideoWrapByRoleAndNode = function(role, node) {
        ...
        clearTimeout(wraoNode.data("timeIdForVideoWrap")), isAgentByRole(role) && wraoNode.removeClass("fn-hide")
        ...
        arr.forEach(function(val) {
            retValue[val] && retValue[val].forEach(function(val, key) {
                val.oldOrder = val.order, val.order = ++key, resConteVal.userId === val.userId && (resConteVal.order = val.order)
            })
        })
    }, trialTool.getRoleNameByRole = function(item, flag) {
        var rightMap = {
                al: "ac",
                dl: "de"
            },
        ...
        return isAgentByRole(item.role) ? firstName + "代理人" : firstName
    }, trialTool.getAllCount = function() {
        return getAllLitigantList.apply(void 0, arguments).length
    };
    var getAllLitigantList = trialTool.getAllLitigantList = function(ac, al, de, dl) {
        var arr = [];
        return pushRightOrder(arr, ac, al), pushRightOrder(arr, de, dl), arr
    };
    return trialTool.getComputeLayout = function() {
        var winWidth = window.innerWidth,
            layoutConfig = {},
            mainWidth = layoutConfig.mainWidth = winWidth - 200,
            bigVideoWidth = layoutConfig.bigVideoWidth = 2 * (mainWidth - 30) / 3,
            bigVideoHeight = layoutConfig.bigVideoHeight = 3 * bigVideoWidth / 4,
            smallVideoHeight = layoutConfig.smallVideoHeight = (bigVideoHeight - 5) / 2;
        layoutConfig.smallVideoWitch = 4 * smallVideoHeight / 3;
        return layoutConfig
        ...
        if ("re" === userInfo.role) {
            var record = tinymce.get("record");
            record ? record.setContent(val) : recordDom.val(val)
        } else {
            var DOC = recordDom[0].contentWindow.document;
            DOC.open(), DOC.write(val), DOC.close(), recordDom.css("height", "auto"), recordDom.height(DOC.documentElement.scrollHeight + 7)
        }
    }, trialTool
});
"use strict";
define("src/bus/suit/trial/trial/trialLayout-debug", ["common/jquery-debug", "common/limit-debug", "common/limit-dom-debug", "common/myWidget-debug", "src/bus/suit/trial/trial/trialTool-debug", "src/bus/suit/trial/trial/trialConstant-debug", "common/tinymce-debug", "src/bus/suit/trial/trial/trial-hbs-debug", "common/handlerbars-debug"], function(require, exports, module) {
    function timer(time) {
        var me = this;
        time = new Date((new Date).getTime() - new Date(time).getTime()), me.$("#lastTime").html("开庭时长 " + [limit.padStart("" + (time.getHours() - 8), "0", 2), limit.padStart("" + time.getMinutes(), "0", 2), limit.padStart("" + time.getSeconds(), "0", 2)].join(":"))
    }
    var $ = require("common/jquery-debug"),
        limit = require("common/limit-debug"),
        ...
        trialHbs = require("src/bus/suit/trial/trial/trial-hbs-debug"),
        TrialLayout = MyWidget.extend({
            ...
            attrs: {
                ...
                store: {
                    pageLink: [{
                        ...
                        key: "evidenceOppugnDetail",
                        value: "质证"
                    }]
                },
                template: {
                    getter: function() {
                        return trialHbs($.extend({
                            layoutConfig: trialTool.getComputeLayout()
                        }, this.get("data"), this.get("store"), trialConstant))
                    }
                }
            },
            events: {},
            ...
            setup: function() {
                var me = this;
                me.get("replaceNode").replaceWith(me.element), "re" === me.get("data").role && tinymceUse({
                    selector: "#record"
                }), me.startTimer()
            },
            startTimer: function(time) {
                var me = this;
                time ? trialTool.setLocalStorageByName("openTime", time) : time = trialTool.getLocalStorageByName("openTime"), void 0 !== time && (clearInterval(me.initTimer), timer.call(me, time), me.initTimer = setInterval(function() {
                    timer.call(me, time)
                }, 1e3))
            }
        });
...
define("src/bus/suit/trial/trial/trial-hbs-debug", ["common/handlerbars-debug"], function(require, exports, module) {
    var Handlerbars = require("common/handlerbars-debug"),
        compile = Handlerbars.compile('<div class="" style="width:{{layoutConfig.mainWidth}}px;margin:0 auto;padding-bottom:50px;">    <div class="global-tab fn-MT10">        <i></i><span class="fn-left">视频庭审现场</span>        <div class="fn-FS12 fn-color-666 last-time fn-hide" id="lastTime">未开庭</div>        <button class="btn small fn-right" id="start">开 庭</button>        <button class="btn small fn-right" id="pause">暂停录制</button>        <div class="fn-right fn-FS12 fn-MR10" id="pageLink">            {{#isEqual role "ju"}}                <span class="icon-mute"></span>可发言，<span class="icon-mute icon-mute-enabled"></span>禁言，可直接点击控制             {{/isEqual}}            {{#each pageLink}}                <a target="_blank" href="/caseDetail/trialCaseDetail.htm?tab={{key}}&securityCaseId={{../sid}}" class="global-link">{{value}}</a>            {{/each}}        </div>        <div class="suit-wrap" >            <span class="fn-color-666 fn-FS14 fn-ML10"><span class="court-name">{{courtName}}</span> <span>在线审理</span><span> <span class="suit-info fn-FS12" title="">{{caseTitle}}</span>        </div>    </div>        <div class="main-list {{#contains role "ju" "ad"}}show-btn manage-only{{/contains}}">        <div class="fixed-video">            <div class="big-win" style="width:{{layoutConfig.bigVideoWidth}}px;" def-img="{{assetsLink}}/assets/img/default.jpg" quit-img="{{assetsLink}}/assets/img/quit.jpg">                    <img style="width:764px;height:572px;overflow:hidden;" src="{{assetsLink}}/assets/img/default.jpg" />            </div><div class="judge-wrap" style="width:{{layoutConfig.smallVideoWitch}}px">                <div class="judge">                    <div class="v-ju1" style="width:{{layoutConfig.smallVideoWitch}}px;height:{{layoutConfig.smallVideoHeight}}px;overflow:hidden;">                        <img style="width:{{layoutConfig.smallVideoWitch}}px;height: 286px" src="{{assetsLink}}/assets/img/default.jpg"/>                    </div>                </div>                <div class="recorder">                    <div class="v-re1" style="width:{{layoutConfig.smallVideoWitch}}px;height:{{layoutConfig.smallVideoHeight}}px;overflow:hidden;">                        <img style="width:{{layoutConfig.smallVideoWitch}}px;height:{{layoutConfig.smallVideoHeight}}px" src="{{assetsLink}}/assets/img/default.jpg"/>                    </div>                </div>            </div>        </div>        <ul class="video-list clearfix">                    </ul>    </div>    <div class="global-tab fn-MT10">        <i></i>庭审笔录直播    </div>    <div class="fn-clear">        {{#isEqual role "re"}}            <textarea name="record" id="record" cols="30" rows="10"></textarea>        {{else}}            <div class="recordBox">                <iframe id="record" width="100%" height="100%"></iframe>            </div>        {{/isEqual}}    </div>        <div id="confirmRecordTitle" style="text-align:center;display:none;" class="fn-color-666 fn-MT10">        您已经确认笔录    </div>        <div class="term" id="confirmRecordCheck">        <input type="checkbox" id="term"> <label for="term">我已阅读笔录，确认无误</label>        <br /><br />        <button class="fn-btn fn-btn-primary" id="confirmRecord">确认笔录</button>    </div>        <div class="term" id="submitRecordUserRe">        <button class="fn-btn fn-btn-primary" id="submitRecord">提 交</button>        <button class="fn-btn fn-btn-primary" id="saveRecord">确认完成</button>    </div>    <div class="record-list" style="text-align:center;">        <div style="display:inline-block;">            <table class="fn-table fn-table-text">                <thead>                    <tr><th>角色</th><th>姓名</th><th>状态</th></tr>                </thead>                <tbody>                </tbody>            </table>        </div>            </div></div>');
    return compile.source = '<div class="" style="width:{{layoutConfig.mainWidth}}px;margin:0 auto;padding-bottom:50px;">    <div class="global-tab fn-MT10">        <i></i><span class="fn-left">视频庭审现场</span>        <div class="fn-FS12 fn-color-666 last-time fn-hide" id="lastTime">未开庭</div>        <button class="btn small fn-right" id="start">开 庭</button>        <button class="btn small fn-right" id="pause">暂停录制</button>        <div class="fn-right fn-FS12 fn-MR10" id="pageLink">            {{#isEqual role "ju"}}                <span class="icon-mute"></span>可发言，<span class="icon-mute icon-mute-enabled"></span>禁言，可直接点击控制             {{/isEqual}}            {{#each pageLink}}                <a target="_blank" href="/caseDetail/trialCaseDetail.htm?tab={{key}}&securityCaseId={{../sid}}" class="global-link">{{value}}</a>            {{/each}}        </div>        <div class="suit-wrap" >            <span class="fn-color-666 fn-FS14 fn-ML10"><span class="court-name">{{courtName}}</span> <span>在线审理</span><span> <span class="suit-info fn-FS12" title="">{{caseTitle}}</span>        </div>    </div>        <div class="main-list {{#contains role "ju" "ad"}}show-btn manage-only{{/contains}}">        <div class="fixed-video">            <div class="big-win" style="width:{{layoutConfig.bigVideoWidth}}px;" def-img="{{assetsLink}}/assets/img/default.jpg" quit-img="{{assetsLink}}/assets/img/quit.jpg">                    <img style="width:764px;height:572px;overflow:hidden;" src="{{assetsLink}}/assets/img/default.jpg" />            </div><div class="judge-wrap" style="width:{{layoutConfig.smallVideoWitch}}px">                <div class="judge">                    <div class="v-ju1" style="width:{{layoutConfig.smallVideoWitch}}px;height:{{layoutConfig.smallVideoHeight}}px;overflow:hidden;">                        <img style="width:{{layoutConfig.smallVideoWitch}}px;height: 286px" src="{{assetsLink}}/assets/img/default.jpg"/>                    </div>                </div>                <div class="recorder">                    <div class="v-re1" style="width:{{layoutConfig.smallVideoWitch}}px;height:{{layoutConfig.smallVideoHeight}}px;overflow:hidden;">                        <img style="width:{{layoutConfig.smallVideoWitch}}px;height:{{layoutConfig.smallVideoHeight}}px" src="{{assetsLink}}/assets/img/default.jpg"/>                    </div>                </div>            </div>        </div>        <ul class="video-list clearfix">                    </ul>    </div>    <div class="global-tab fn-MT10">        <i></i>庭审笔录直播    </div>    <div class="fn-clear">        {{#isEqual role "re"}}            <textarea name="record" id="record" cols="30" rows="10"></textarea>        {{else}}            <div class="recordBox">                <iframe id="record" width="100%" height="100%"></iframe>            </div>        {{/isEqual}}    </div>        <div id="confirmRecordTitle" style="text-align:center;display:none;" class="fn-color-666 fn-MT10">        您已经确认笔录    </div>        <div class="term" id="confirmRecordCheck">        <input type="checkbox" id="term"> <label for="term">我已阅读笔录，确认无误</label>        <br /><br />        <button class="fn-btn fn-btn-primary" id="confirmRecord">确认笔录</button>    </div>        <div class="term" id="submitRecordUserRe">        <button class="fn-btn fn-btn-primary" id="submitRecord">提 交</button>        <button class="fn-btn fn-btn-primary" id="saveRecord">确认完成</button>    </div>    <div class="record-list" style="text-align:center;">        <div style="display:inline-block;">            <table class="fn-table fn-table-text">                <thead>                    <tr><th>角色</th><th>姓名</th><th>状态</th></tr>                </thead>                <tbody>                </tbody>            </table>        </div>            </div></div>', compile
});
"use strict";
