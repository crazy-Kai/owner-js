"use strict";
define("src/bus/suit/trial/checkFocus/main-debug", ["bus/global/main-debug", "bus/suit/trial/global/main-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "common/domUtil-debug", "common/jquery-debug", "common/handlerbars-debug", "model/modal/main-debug"], function(require, exports, module) {
    require("bus/global/main-debug"), require("bus/suit/trial/global/main-debug");
    var url, util = require("common/util-debug"),
        domUtil = require("common/domUtil-debug"),
        Modal = require("model/modal/main-debug"),
        caseId = util.getUrlParam("caseId");
    caseId && (url = $("#prev").attr("href"), url = url.indexOf("?") == -1 ? url + "?caseId=" + caseId : url + "&caseId=" + caseId, $("#prev").attr("href", url)), $("#notic-check-next").click(function(e) {
        return e.preventDefault(), domUtil.isWebkit ? void(caseId ? $("#trial-checked").prop("checked") && (window.location.href = "/suit/trial/trialStart.do?caseId=" + caseId) : Modal.alert(0, "没有案件id")) : Modal.confirm("请使用Google Chrome浏览器进入庭审", '目前网上法庭在线视频庭审只支持Google Chrome浏览器，需要进入庭审请使用Google Chrome浏览器，<a class="global-link" target="_blank" href="http://dlsw.baidu.com/sw-search-sp/soft/9d/14744/ChromeStandalone_V45.0.2454.99_Setup.1442891522.exe">下载地址</a>', null, null, {
            width: 500,
            noCancle: !0,
            closeTpl: "",
            noSure: !0
