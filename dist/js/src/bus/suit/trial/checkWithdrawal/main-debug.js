"use strict";
define("src/bus/suit/trial/checkWithdrawal/main-debug", ["bus/global/main-debug", "common/jquery-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "model/modal/main-debug"], function(require, exports, module) {
    require("bus/global/main-debug");
    var url, $ = require("common/jquery-debug"),
    ...
    caseId && (url = $("#prev").attr("href"), url = url.indexOf("?") == -1 ? url + "?caseId=" + caseId : url + "&caseId=" + caseId, $("#prev").attr("href", url)), $("#notic-check-next").click(function(e) {
        e.preventDefault(), caseId ? "1" == $("input[name=diffect]:checked").val() ? Modal.alert(0, "提出异议, 请联系审判法官") : (url = $("#notic-check-next").attr("href"), url = url.indexOf("?") == -1 ? url + "?caseId=" + caseId : url + "&caseId=" + caseId, window.location.href = url) : Modal.alert(0, "没有案件id")
    })
});
"use strict";
