"use strict";
define("src/bus/suit/trial/checkEnvironment/main-debug", ["bus/global/main-debug", "bus/suit/trial/global/main-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "model/modal/main-debug", "common/cookie-debug"], function(require, exports, module) {
    ...
    caseId ? (url = $("#notic-check-next").attr("href"), url = url.indexOf("?") == -1 ? url + "?caseId=" + caseId : url + "&caseId=" + caseId, $("#notic-check-next").attr("href", url), cookie.get("openText") || $("#notic-check-next").click(function(e) {
        var node = $(this);
        e.preventDefault(), cookie.set("openText", "true"), Modal.show("/suit/trial/trialTest.htm?caseId=" + $("#caseId").val() + "&entityRole=" + $("#entityRole").val(), {
            width: 1200,
            height: 950,
            closeTpl: '<button class="fn-btn fn-btn-default" type="button">下一步</button>'
        }).after("hide", function() {
            location.href = node.prop("href")
        })
    })) : $("#notic-check-next").click(function(e) {
        e.preventDefault(), Modal.alert(0, "没有案件id")
    })
});
