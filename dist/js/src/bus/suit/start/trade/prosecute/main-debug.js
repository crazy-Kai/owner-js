"use strict";
define("src/bus/suit/start/trade/prosecute/main-debug", ["bus/global/main-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "common/domUtil-debug", "common/jquery-debug", "common/handlerbars-debug", "model/perSubmit/main-debug", "model/modal/main-debug", "common/validator-debug", "common/tip-debug", "model/perChecked/main-debug"], function(require, exports, module) {
    function isOverFlow() {
        var amount = 0,
            msg = null;
        return util.breakEachArr(domUtil.serialize("#suitRequest").lassenSuitRequestDoList, function(val) {
            amount += +val.amount
    ...
    var vaExp, util = require("common/util-debug"),
        ...
        PerSubmit = require("model/perSubmit/main-debug"),
    ...
    new PerChecked({
        ...
        if (flag) {
            ...
            var msg = isOverFlow();
            if (msg) return Modal.alert(0, msg);
            if ("网络购物合同纠纷" === causeAction.val() && PerSubmit.getWidget("#selectionOrder").soAccusedGuid > 0) return Modal.alert(0, "由于案由是网络购物合同，请把添加的被告删除。");
            new PerSubmit({
                element: "#page-form"
    ...
    causeAction.on("change", function(e, reason) {
        ...
        reason && util.breakEachArr(list, function(val) {
            val.value === reason && (val.selected = !0)
        }), $("#selectionOrder .JS-trigger-click-add")["网络购物合同纠纷" === val ? "addClass" : "removeClass"]("fn-btn-disabled"), domUtil.selectSerialize(selectReason[0], list), tipMsg.set("content", causeActionMsgData[val])
    }), causeAction.trigger("change", selectReason.data("reason")), window.onbeforeunload = function(e) {
