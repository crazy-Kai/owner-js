"use strict";
define("src/bus/suit/start/paymentOrder/prosecute/main-debug", ["bus/global/main-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "common/domUtil-debug", "common/jquery-debug", "common/handlerbars-debug", "model/ajax/main-debug", "model/modal/main-debug", "common/validator-debug", "model/perChecked/main-debug"], function(require, exports, module) {
    function isOverFlow() {
        var amount = 0,
            msg = null;
        return util.breakEachArr(domUtil.serialize("#page-allinfo").lassenSuitRequestDoList, function(val) {
            val.amount && (amount += +val.amount)
        }), util.log("log", amount), amount > 5e6 ? msg = "请求赔偿金额超过500万，请前往法院进行线下起诉！" : isNaN(amount) && (msg = "填写的金额有错误"), msg
    }

    function verify() {
        var table, entityType = $('[name="paySuitEntityDo.entityType"]').val(),
            table = $('[data-role="' + entityType + '"]');
        return $('[data-role="verify"]').addClass("fn-hide"), table.find('[name="paySuitEntityDo.mobile"]').val() || table.find('[name="paySuitEntityDo.email"]').val() ? (table.find('[data-role="verify"]').addClass("fn-hide"), !1) : (table.find('[data-role="verify"]').removeClass("fn-hide"), !0)
    }

    function formValidator() {
        var flag;
        return vaExp && vaExp.destroy(), vaExp = Validator.use("#page-allinfo", ".JS-prosecute-required"), vaExp.addItem({
            element: '[name="basisContent"]',
            skipHidden: !1
            ...
            flag = isError, isError && util.log(errList)
        }), verify() || flag
    }
    ...
    var vaExp, util = require("common/util-debug"),
    ...
    new PerChecked({
        ...
        if (flag) {
            ...
            if ($("#page-allinfo").find('input[type="text"]').removeClass("fn-hide"), isError) return;
            var msg = isOverFlow();
            msg ? Modal.alert(0, msg) : new Ajax({
                element: "#page-form",
                paramParse: function(json) {
                    ...
                    return limit.each(obj, function(v, i) {
                        verMap[i] && (obj[verMap[i]] = ""), obj[i].indexOf("*") !== -1 && delete obj[i]
                    }), json.fact = JSON.stringify(json.fact), json.tips = JSON.stringify(json.tips), json
                }
            }).on("ajaxSuccess", function() {
                var me = this,
                    DO = me.serialize("#pageParam");
                isGo = !1, me.redirect(me.get("jump") + me.getEscapeUrl(DO.securityId))
            }).submit()
        }
