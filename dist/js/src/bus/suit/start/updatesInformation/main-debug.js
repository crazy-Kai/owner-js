"use strict";
define("src/bus/suit/start/updatesInformation/main-debug", ["bus/global/main-debug", "common/jquery-debug", "common/delegate-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "common/dialog-debug", "common/domUtil-debug", "common/handlerbars-debug", "common/validator-debug", "common/scroller-debug", "common/calendar-debug", "model/upload/main-debug", "model/ajax/main-debug", "common/tip-debug", "model/imgView/main-debug", "model/modal/main-debug", "model/addLawyer/main-debug"], function(require, exports, module) {
    function toggleBtn(node, btn) {
        node.prop("checked") ? btn.removeClass("fn-btn-disabled") : btn.addClass("fn-btn-disabled")
    }

    function hiddenPage() {
        noticeCheckPage.addClass("fn-hide"), updatesInformationPage.addClass("fn-hide"), chooseTypePage.addClass("fn-hide")
    }

    function showPage(node) {
        hiddenPage(), node.removeClass("fn-hide"), document.documentElement.scrollTop = 0, document.body.scrollTop = 0, document.title = node.data("title"), isGo = node[0] === updatesInformationPage[0]
    }
    ...
    function checkedFalse() {
        $(".JS-trigger-hover").removeClass("trigger-chosed").addClass("JS-trigger-chose").removeClass("JS-trigger-chosed"), $('#chooseType-page [type="radio"]').prop("checked", !1)
    }
    ...
    function setLigintInfor(rtv) {
        ...
        $("#selectedLigintDetails").html(template(rtv)), rtv.lassenLitigantAgentDto || (rtv.lassenLitigantAgentDto = {}), rtv.lassenLitigantAgentDto.inputName = "relationFile", getRelationWithLigint(rtv.entityType, rtv.lassenLitigantAgentDto), Upload.use("#selectedLigintDetails .JS-need-upload"), validatorExp.addItems("#selectedLigintDetails"), $('[data-identifytype="agentDetails"] .JS-trigger-add-lawyer').closest("tr").next("tr").remove(), $('[data-identifytype="agentDetails"] .JS-trigger-add-lawyer').nextAll("#addLawyerNote").remove(), $('[data-identifytype="agentDetails"] .JS-trigger-add-lawyer').addClass("fn-btn-default").removeClass("fn-btn-disabled"), Calendar.use()
    }
    ...
    function getRelationWithLigint(entityType, relationFileDo) {
        ...
        "normal" == entityType ? request = "/suit/lassenLitigantAgentRpc/getJSONByType.json?optionType=LIGINT_RELATION_NORMAL" : "legal" == entityType && (request = "/suit/lassenLitigantAgentRpc/getJSONByType.json?optionType=LIGINT_RELATION_LEGAL"), request && new Ajax({
            ...
            if ($('select[name="relationCode"]').html(template(JSON.parse(rtv))), relationFileDo) {
                var relationCode = relationFileDo.relationCode;
                relationCode ? $('[name="relationCode"] option[value="' + relationCode + '"]').prop("selected", !0) : $('[name="relationCode"] option:first').prop("selected", !0);
                var relationHand = handlerbars.compile($("#a-common-relationfile-template"), !0);
                $('[data-role="commonRelationFile"]').html(relationHand(relationFileDo)), Upload.use('[data-role="commonRelationFile"] .JS-need-upload'), $('[data-identifytype="agentDetails"] [name="relationCode"]').trigger("change")
            }
        }).submit()
    }
    ...
    var pageForm = $("#page-form"),
        ...
        validatorExp = Validator.use("#updatesInformation-form", ".JS-target-required");
    "ligint" === $('[data-role="identifyType"]').val() ? validatorExp.removeItems("[data-identifytype='agentDetails']") : validatorExp.removeItems("[data-identifytype='ligintDetails']"), new ImgView, Calendar.use(), Upload.use(".JS-need-upload"), errorMsg && Modal.confirm("提醒", errorMsg, function() {
        domUtil.redirect("/suit/newMySuit.htm#personInfo")
    }, function() {
        domUtil.redirect("/portal/main/domain/index.htm")
    }), Tip.use();
    var scroller;
    setTimeout(resetForm, 0), delegate.on("mouseenter", ".JS-trigger-hover", function() {
        var node = $(this);
        ...
        checkedFalse(), target.addClass("trigger-chosed").addClass("JS-trigger-chosed"), target.find('[type="radio"]').prop("checked", !0)
    }), delegate.on("click", ".JS-trigger-chosed", checkedFalse), delegate.on("click", ".JS-trigger-click-detail", function() {
        Modal.show(noticeCheckTemple, {
            width: 600
        })
        ...
        checkNodeCanUse($(this)) && (showPage(chooseTypePage), scroller || (scroller = Scroller.use(".JS-need-scroller")))
    }), chooseTypePrev.on("click", function() {
        showPage(noticeCheckPage)
    });
    var isGo = !1;
    chooseTypeNext.on("click", function() {
        return chooseTypePage.find('[type="radio"]:checked').length ? void showPage(updatesInformationPage) : (Modal.alert(0, "请选择起诉类型"), !1)
        ...
        checkNodeCanUse($(this)) && (tempHideInout(), validatorExp.execute(function(err, errlist) {
            updatesInformationPage.find('input[type="text"]').removeClass("fn-hide"), err ? util.log(errlist) : new Ajax({
                element: "#page-form",
                needText: !0,
                paramParse: function(json) {
                    for (var i in json) {
                        if ("lassenSuitEntityDo" === i)
                            for (var j in json[i]) json[i][j].indexOf("*") !== -1 && delete json[i][j];
                        json[i] = JSON.stringify(json[i])
                    }
                    return json
                }
            }).on("ajaxSuccess", function(rtv, msg, con) {
                isGo = !1, domUtil.redirect(chooseTypePage.find('[type="radio"]:checked').attr("data-jump") + "?securityId=" + domUtil.getEscapeUrl(rtv.securityId))
            }).on("ajaxError", function() {
                isBindUse && bussinessBtn.prop("checked", !1), isBindUse = !1
            }).submit()
        }))
    }), window.onbeforeunload = function(e) {
        e = e || event, isGo && (e.returnValue = "您确定要离开此页面么？")
    };
    var isBindUse, bussinessBtn = $('#chooseType-page input[name="lassenLegalCaseDo.caseType"]').eq(0);
    delegate.on("click", ".JS-bind-bussinedss", function() {
        window.bindCallback = function() {
            bussinessBtn.prop("checked", !0), $("#chooseType-next").trigger("click"), isBindUse = !0
        }, Dialog.show("/havlogin.htm?app=taobao", {
            width: 380,
            height: 380
        })
    }), delegate.on("change", '[data-role="identifyType"]', function(e) {
        var type = $(e.target).val();
        "ligint" === type ? ($('div[data-role="agent-note" ]').addClass("fn-hide"), $('div[data-role="ligint-note" ]').removeClass("fn-hide"), $('[data-identifytype="agentDetails"]').addClass("fn-hide JS-serialize-exclude"), $('[data-identifytype="ligintDetails"]').removeClass("fn-hide JS-serialize-exclude"), validatorExp.removeItems("[data-identifytype='agentDetails']"), validatorExp.addItems("[data-identifytype='ligintDetails']")) : ($('div[data-role="agent-note" ]').removeClass("fn-hide"), $('div[data-role="ligint-note" ]').addClass("fn-hide"), $('[data-identifytype="ligintDetails"]').addClass("fn-hide JS-serialize-exclude"), $('[data-identifytype="agentDetails"]').removeClass("fn-hide JS-serialize-exclude"), validatorExp.removeItems("[data-identifytype='ligintDetails']"), validatorExp.addItems("[data-identifytype='agentDetails']"))
    }), delegate.on("change", '[data-identifytype="agentDetails"] [name="relationCode"]', function(e) {
        var type = $(e.target).val();
        "normal_lawyer" === type || "legal_lawyer" === type ? ($('[data-role="commonRelationFileTr"]').addClass("fn-hide JS-serialize-exclude"), validatorExp.removeItems('[data-role="commonRelationFile"]')) : $('[data-role="commonRelationFileTr"]').removeClass("fn-hide JS-serialize-exclude")
    ...
    delegate.on("click", '[data-role="selectLigint"]', function(e) {
        ...
        "alipay" === type ? alipayDialog = Dialog.show("/suit/start/ligintAlipay.htm?", {
            width: 380,
            height: 380
        }) : new Ajax({
