"use strict";
define("src/bus/court/myCase/main-debug", ["bus/global/main-debug", "common/jquery-debug", "common/delegate-debug", "model/countDown/main-debug", "common/calendar-debug", "model/searchList/main-debug", "model/ajax/main-debug", "model/modal/main-debug", "common/validator-debug", "model/advancedQuery/main-debug", "model/switchFilterBtn/main-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "common/dialog-debug", "common/handlerbars-debug", "common/statusMap-debug", "common/paymentStatusMap-debug"], function(require, exports, module) {
    ...
    var $ = require("common/jquery-debug"),
        ...
        Calendar = (require("model/countDown/main-debug"), require("common/calendar-debug")),
        ...
        Handlebars = require("common/handlerbars-debug");
    ...
    var calendars = {
        ...
        searchListExp = (Validator.use("#mycase-form", ".JS-target-date"), SearchList.use(".searchList", {
            map: function(data) {
                for (var i = 0; i < data.length; i++) data[i].status && ("payment_order" === data[i].caseType ? (data[i].statusEx = paymentStatusMap[data[i].status], data[i].amount = util.formatMoney(data[i].amount, 2)) : (data[i].statusEx = statusMap[data[i].status], data[i].amount = util.formatMoney(data[i].amount, 2)));
                return data
            }
        }));
    new advancedQuery, delegate.on("click", "#search", function() {
        ...
        }), $('[data-role="class"]').trigger("change"), $("#submit-date-fr, #submit-date-to, #register-date-fr, #register-date-to, #judge-date-fr, #judge-date-to").prop("disabled", "")
    }), new SwitchFilterBtn({
        ...
        "javascript" == action ? Modal.confirm("提醒", "请确认是否不再庭审，确认后案件将不能再庭审", function() {
            new Ajax({
        ...
        }) : "trial_court_send_message" == action && Modal.confirm("提醒", "是否确认提醒当事人？", function() {
            new Ajax({
                request: "/court/CourtRpc/sendPutProofEndMessage.json?securityId=" + encodeURIComponent($(e.target).data("securityid")),
                autoSuccessAlert: !0
            }).on("ajaxSuccess", function(rtv, msg, con) {}).submit()
        })
    }), delegate.on("click", ".child-nav li", function() {
        $(this).addClass("active").siblings().removeClass("active")
    }), $('select[data-role="class"]').trigger("change"), new Ajax({
        element: "#suit-content",
        autoDestroy: !1,
        events: {
            'click [data-role="putProofEnd"]': function(e) {
                var me = this;
                e.preventDefault(), me.set("request", "/account/mySuitRpc/putProofEnd.json"), me.set("param", $(e.target).data("param")), Modal.confirm("结束举证期限", "请确认是否提前结束举证", function() {
                    me.submit()
                }, null, {
                    sureBtnText: "确认结束"
                })
            ...
            'click [data-role="putProofDayDelay"]': function(e) {
                ...
                for (var param = $(e.target).data("param"), list = param.list = [], i = 0; i < param.putProofDayDelayMax - ~~param.putProofDayDelay;) list[i] = ++i;
                1 === list.length && (param.one = !0), me.dia = Dialog.showTemplate("#chose-day", param, {
                    width: 240,
                    events: {
                        'click [data-role="submit"]': function() {
                            me.set("parseForm", this.element), me.submit()
                        }
                    }
                })
            }
