define("src/bus/domain/personCenter/main-debug", ["bus/global/main-debug", "common/jquery-debug", "common/limit-debug", "common/limit-dom-debug", "common/dialog-debug", "model/ajax/main-debug", "common/util-debug", "common/promise-debug", "model/modal/main-debug", "common/handlerbars-debug", "common/validator-debug", "model/poploading/main-debug", "common/widget-debug", "common/base-debug", "common/class-debug", "common/attrs-debug", "common/aspect-debug", "common/events-debug"], function(require, exports, module) {
    function beZero(num) {
        return num < 10 ? "0" + num : num
    }

    function format(date, separator) {
        return separator = separator || ".", date.getFullYear(date) + separator + beZero(date.getMonth() + 1) + separator + beZero(date.getDate())
    }
    ...
    $.ajaxSetup({
        ...
        cb: function(res) {
            function addValidator() {
                $(".JS-basic-info table").on("blur", 'input[type="text"]', function() {
                    var self = $(this);
                    self.prop("defaultValue") === self.prop("value") && self.addClass("fn-hide")
            ...
            function removeValidator() {
                $(".JS-basic-info table").off("blur"), $("body").off("blur"), validatorExp.removeItem("[name=name]").removeItem("[name=mailbox]").removeItem("[name=postalcode]").removeItem("[name=legalPerson]").removeItem("[name=nationality]").removeItem("[name=currentAddress]").removeItem("[name=homeAddress]").removeItem("[name=profession]").removeItem("[name=linkman]").removeItem("[name=registeredAddress]").removeItem("[name=mailingAddress]").removeItem("[name=aliim]")
            }

            function initInfo() {
                removeValidator(), "normal" == personalCenterVo.type ? (accountNormal && (accountNormal.gender && (accountNormal.genderEx = "male" == accountNormal.gender ? "男" : "女"), accountNormal.mobileEx = accountNormal.mobile || "", accountNormal.telEx = accountNormal.tel || "", delete accountNormal.birthday), template.normalInfo = template.normalInfo || handlerbars.compile($("#template-normal-info").html()), $(".JS-basic-info").html(template.normalInfo(personalCenterVo))) : "legalPerson" == personalCenterVo.type && (legalPerson && (legalPerson.idcardNumberEx = legalPerson.idcardNumber || "", legalPerson.mobileEx = legalPerson.mobile || "", legalPerson.telEx = legalPerson.tel || "", legalPerson.typeEx = legalPersonTypeMap[legalPerson.type], legalPerson.idcardTypeEx = idcardTypeMap[legalPerson.idcardType]), template.legalInfo = template.legalInfo || handlerbars.compile($("#template-legal-info").html()), $(".JS-basic-info").html(template.legalInfo(personalCenterVo))), editing = !1, $(".JS-edit-info").text("编辑信息")
            }
            ...
            function initCertification(accountCertification) {
                var certification = !1;
                accountCertification && (accountCertification.idcardTypeEx = idcardTypeMap[accountCertification.idcardType], accountCertification.idcardNumberEx = accountCertification.idcardNumber || "", accountCertification.expireDateEx = accountCertification.expireDateStart ? format(new Date(accountCertification.expireDateStart)) + "-" + format(new Date(accountCertification.expireDateEnd)) : "", $(".person-center-status .card-number").text(accountCertification.idcardNumberEx), certification = !0), template.accountCertification = handlerbars.compile($("#template-account-certification").html()), $(".JS-validate-info").html(template.accountCertification({
                    lassenAccountCertificationDo: accountCertification,
                    certification: certification
                }))
            }
            var personalCenterVo, editing = !1,
                template = {},
                legalPersonTypeMap = {
                    enterprise: "企业",
                    association: "协会",
            ...
            if (res.hasError) Modal.alert(0, "加载页面出错");
            else {
                if (personalCenterVo = res.content.retValue, personalCenterVo.hasData = !(!personalCenterVo.lassenAccountNormalDo && !personalCenterVo.lassenAccountLegalPersonDo), personalCenterVo.lassenAccountLegalPersonDo = personalCenterVo.lassenAccountLegalPersonDo || {}, personalCenterVo.lassenAccountNormalDo = personalCenterVo.lassenAccountNormalDo || {}, personalCenterVo.lassenAccountCertificationDo = personalCenterVo.lassenAccountCertificationDo || {}, !personalCenterVo) return;
                var legalPerson = personalCenterVo.lassenAccountLegalPersonDo,
                    accountNormal = personalCenterVo.lassenAccountNormalDo;
                personalCenterVo.lassenAccountCertificationDo;
                "certification" == personalCenterVo.status && (personalCenterVo.idcardNumber = personalCenterVo.idcardNumber || "", personalCenterVo.openaccountId = personalCenterVo.openaccountId || ""), template.childInfo = handlerbars.compile($("#template-child-info").html()), $(".child-info").html(template.childInfo(personalCenterVo));
                var validatorExp = new Validator({
                    element: ".JS-basic-info",
                    failSilently: !0,
                    skipHidden: !0
                });
                $(".JS-edit-info").click(function(e) {
                    if (e.preventDefault(), editing) initInfo();
                    else {
                        if (!personalCenterVo.hasData) return;
                        $(this).text("取消编辑"), "normal" == personalCenterVo.type ? (template.normalInfoEdit = template.normalInfoEdit || handlerbars.compile($("#template-normal-info-edit").html()), $(".JS-basic-info").html(template.normalInfoEdit(personalCenterVo))) : "legalPerson" == personalCenterVo.type && (template.legalInfoEdit = template.legalInfoEdit || handlerbars.compile($("#template-legal-info-edit").html()), $(".JS-basic-info").html(template.legalInfoEdit(personalCenterVo))), editing = !0, addValidator()
                    }
                }), $(".JS-basic-info").on("click", ".JS-submit-edit", function(e) {
                    hiddenInputInNeed(), validatorExp.execute(function(flag, element) {
                        showInputInNeed(), flag || new Ajax({
                            request: "/account/accountRpc/submitAccount.json",
                            paramName: "personCenterVo",
                            parseForm: ".JS-basic-info",
                            paramParse: function(json) {
                                return limit.each(json, function(val) {
                                    limit.each(val, function(v, k) {
                                        v.indexOf("*") !== -1 && delete val[k]
                                    })
                                }), json
                            }
                ...
                for (var accountBinding = personalCenterVo.accountBindingDoList || [], sourceMap = {
                        asip: "知识产权",
                        taobao: "淘宝",
                        alipay: "支付宝"
                    }, i = 0; i < accountBinding.length; i++) accountBinding[i].sourceEx = sourceMap[accountBinding[i].source];
                template.accountBinding = handlerbars.compile($("#template-bind-account").html()), $(".JS-bind-account").html(template.accountBinding(personalCenterVo)), $(".JS-bind-account").on("click", ".JS-trigger-unbind", function(e) {
                    ...
                    Modal.confirm("提示 ", "解绑后，如再起诉需重新绑定电商账号。", function() {
                        ajax({
                            ...
                            url: url,
                            cb: function(res) {
                                res.hasError ? Modal.alert(0, "解除关联失败") : res.content.isSuccess ? window.location.reload() : Modal.alert(0, res.content.message)
                            }
                        })
                    })
                });
                var accountCertification = personalCenterVo.lassenAccountCertificationDo;
                initCertification(accountCertification), $(".JS-validate-info").on("click", ".JS-certificate", function(e) {
                    e.preventDefault(), Dialog.show("/havCeresLogin.htm", {
                        width: 380,
                        height: 380
                    })
                })
        ...
        e.preventDefault(), Dialog.show($(this).attr("href"), {
            width: 380,
            height: 380
        })
    })
