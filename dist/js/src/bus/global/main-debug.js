"use strict";
define("src/bus/global/main-debug", ["common/jquery-debug", "common/constant-debug", "common/limit-debug", "common/limit-dom-debug", "common/domUtil-debug", "common/util-debug", "common/promise-debug", "common/handlerbars-debug", "common/dialog-debug", "common/delegate-debug", "model/placeHolder/main-debug", "model/imgView/global-debug", "model/postMessage/main-debug", "model/mainnav/main-debug"], function(require, exports, module) {
    function maxlength() {
        var self = $(this),
    ...
    function setAmount() {
        ...
        setTimeout(function() {
            var val = self.val();
            val && !REX_AMOUNT.test(val) && self.val(val.match(REX_AMOUNT_GET) ? val.match(REX_AMOUNT_GET)[0] : "")
        }, 0)
    }
    ...
    var $ = require("common/jquery-debug"),
        constant = require("common/constant-debug"),
    ...
    if (new(require("model/mainnav/main-debug")), $("[placeholder]").each(function() {
            new PlaceHolder({
                element: this,
                shadowTextCss: {
                    left: "10px",
            ...
            13 === e.charCode && domUtil.redirect(constant.getUrl("suitCaseSearch") + "?caseInfo=" + encodeURIComponent($(".JS-trigger-keypress-search").val()))
            ...
            var dialogIframe = WIN.dialogIframe = dialog.show("/login.htm", {
                    width: 380,
                    height: 380
                }).after("hide", function() {
            ...
            postMessageExp.add(function(data) {
                "successLogin" === data.type && ($("#login-success").after('<span><a href="/suit/newMySuit.htm#personInfo" target="_blank" class="fn-color-0073bf underline">' + data.userName + '</a></span> <a href="/loginOut.do" class="underline">退出</a>'), $("#login-success").remove(), $(".fast_register").remove(), dialogIframe.hide())
            })
        }), !documentMode || 8 !== documentMode && 9 !== documentMode || (delegate.on("keydown", "[maxlength]", maxlength), delegate.on("paste", "[maxlength]", maxlength)), delegate.on("keydown", '[name="amount"],[data-type="amount"]', setAmount), delegate.on("paste", '[name="amount"],[data-type="amount"]', setAmount), !domUtil.isWebkit) {
        ...
        $("body").prepend(node), node.animate({
            height: 25
        }), $(".bodyer").css({
            "min-height": "calc(100vh - 365px)"
        })
    }
    checkPhoneUser() && alert("建议在电脑上使用Chrom访问本站，以保证显示正确")
});
"use strict";
