"use strict";
define("src/bus/common/loginSuc/main-debug", ["common/jquery-debug", "common/domUtil-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "common/handlerbars-debug", "common/constant-debug", "model/postMessage/main-debug"], function(require, exports, module) {
    function reWriteIntro() {
        WIN.jQuery("#login-success").after('<span><a href="/suit/newMySuit.htm#personInfo" target="_blank" class="fn-color-0073bf underline">' + userName.val() + '</a></span> <a href="/loginOut.do" class="underline">退出</a>'), WIN.jQuery("#login-success").remove(), WIN.jQuery(".fast_register").remove()
    }

    function successLogin() {
        if (self !== top) try {
            reWriteIntro(), $("#redirectUrl").val() ? setTimeout(function() {
                WIN.location.href = $("#redirectUrl").val()
            }, 300) : setTimeout(function() {
                WIN.dialogIframe && WIN.dialogIframe.hide(), delete WIN.dialogIframe
            }, 300)
        } catch (e) {
            postMessageExp.post({
                type: "successLogin",
            ...
            var url = "";
            if (url = constant.getUrl("domainIndex"), $("#redirectUrl").val()) url = $("#redirectUrl").val();
            else {
                var fromUrl = window.location.search;
                fromUrl && fromUrl.lastIndexOf("backUrl=") > 0 && (fromUrl = fromUrl.substring(fromUrl.lastIndexOf("backUrl=") + 8, fromUrl.length)) && (url = decodeURIComponent(fromUrl))
            }
            setTimeout(function() {
    ...
    var $ = require("common/jquery-debug"),
        constant = (require("common/domUtil-debug"), require("common/constant-debug")),
        PostMessage = require("model/postMessage/main-debug"),
        postMessageExp = new PostMessage,
        userName = ($("#login-msg"), $("#userName")),
        WIN = window.top;
    successLogin()
});
"use strict";
