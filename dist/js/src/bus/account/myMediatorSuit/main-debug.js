"use strict";
define("src/bus/account/myMediatorSuit/main-debug", ["bus/global/main-debug", "common/jquery-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "model/filterConditions/main-debug", "model/modal/main-debug", "model/countDown/main-debug", "common/dialog-debug", "common/domUtil-debug", "common/handlerbars-debug", "model/ajax/main-debug", "model/switchFilterBtn/main-debug", "model/searchList/main-debug", "common/statusMap-debug"], function(require, exports, module) {
    function doSearch() {
    ...
    var searchListExp = SearchList.use(".searchList", {
        map: function(data) {
            for (var i = 0; i < data.length; i++) data[i].status && (data[i].statusEx = statusMap[data[i].status]), data[i].amount = data[i].amount.toFixed(2);
            return data
        }
    });
    new SwitchFilterBtn({
