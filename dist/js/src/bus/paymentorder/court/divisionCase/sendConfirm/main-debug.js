"use strict";
define("src/bus/paymentorder/court/divisionCase/sendConfirm/main-debug", ["bus/global/main-debug", "common/jquery-debug", "src/bus/paymentorder/court/divisionCase/sendConfirm/sendConfirm-hbs-debug", "common/handlerbars-debug", "common/validator-debug", "common/calendar-debug", "model/upload/main-debug", "common/domUtil-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "model/ajax/main-debug", "common/tip-debug", "model/modal/main-debug", "common/dialog-debug"], function(require, exports, module) {
    require("bus/global/main-debug");
    var $ = require("common/jquery-debug"),
        sendConfirmHbs = require("src/bus/paymentorder/court/divisionCase/sendConfirm/sendConfirm-hbs-debug"),
    ...
    var sendConfirm = Dialog.show(sendConfirmHbs({}), {
            ...
            events: {
                'click [data-role="sure"]': function(e) {
                    var confirmDetial = $('[name="lassenDocumentConfirmDo.content"]').val();
                    $('[name="lassenDocumentConfirmDo.content"]').val(confirmDetial.replace(/^\s*$/, "")), validatorExp.execute(function(flag, err) {
                        flag || new Ajax({
                            ...
                            paramParse: function(json) {
                                for (var i in json) "object" == typeof json[i] && (json[i] = JSON.stringify(json[i]));
                                return json
                            }
            ...
            var me = this;
            domUtil.resetForm(me.$("#sendConfirm-form")), validatorExp.clearError(), upload[0].uploadRenderClear()
        }).render(),
        validatorExp = Validator.use("#sendConfirm-form"),
        upload = Upload.use(".JS-need-upload");
    $('[data-role="sendConfirm"]').on("click", function() {
        ...
        new Ajax({
            ...
            Model.alert(1, "发送成功"), location.reload()
        }).submit()
    }), new Calendar({
