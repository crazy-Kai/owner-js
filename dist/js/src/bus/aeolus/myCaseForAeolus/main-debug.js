"use strict";
define("src/bus/aeolus/myCaseForAeolus/main-debug", ["bus/global/main-debug", "common/jquery-debug", "common/delegate-debug", "common/limit-debug", "common/limit-dom-debug", "common/util-debug", "common/promise-debug", "model/filterConditions/main-debug", "model/modal/main-debug", "model/countDown/main-debug", "common/dialog-debug", "common/domUtil-debug", "common/handlerbars-debug", "model/ajax/main-debug", "model/searchList/main-debug", "transfer-debug", "common/validator-debug", "src/bus/aeolus/myCaseForAeolus/secondarymenu-debug", "common/myWidget-debug", "common/statusMap-debug", "common/paymentStatusMap-debug"], function(require, exports, module) {
    function doSearch() {
    ...
    var $ = require("common/jquery-debug"),
        ...
        Modal = (require("common/util-debug"), require("model/filterConditions/main-debug"), require("model/modal/main-debug")),
        Dialog = (require("model/countDown/main-debug"), require("common/dialog-debug")),
        domUtil = require("common/domUtil-debug"),
        SearchList = (require("model/ajax/main-debug"), require("model/searchList/main-debug")),
        Transfer = require("transfer-debug"),
        Validator = require("common/validator-debug"),
        Handlerbars = require("common/handlerbars-debug"),
        Secondarymenu = require("src/bus/aeolus/myCaseForAeolus/secondarymenu-debug");
    require("common/statusMap-debug"), require("common/paymentStatusMap-debug"), Validator.use("#mycase-form", ".JS-target-date");
    $.ajaxSetup({
        cache: !1
    }), Handlerbars.registerHelper("isNeedPop", function(isFlag, status, options) {
        return "n" !== isFlag && "not_accepted" !== status && "unpaid_dropped" !== status && "return" !== status && "dropped" !== status && "jurisediction_objection" !== status && "conciliate" !== status && "not_be_served" !== status && "cancel_apply" !== status && "sentenced" !== status ? options.fn(this) : options.inverse(this)
    });
    var searchListExp = SearchList.use(".searchList", {
        map: function(data) {
            var res = {};
            return data.length && (res.hasRes = !0, res.keys = colNames.val().split(","), res.list = data, res.width = 160 * res.keys.length), res
        }
    });
    new Secondarymenu, delegate.on("click", "#search", function() {
        doSearch()
    }), delegate.on("click", "#reset", function() {
        $(":reset").trigger("click"), $('[data-role="class"]').trigger("change")
    });
...
"use strict";
define("src/bus/aeolus/myCaseForAeolus/secondarymenu-debug", ["common/myWidget-debug", "model/ajax/main-debug", "common/handlerbars-debug", "common/limit-debug", "common/limit-dom-debug", "common/domUtil-debug", "common/jquery-debug", "common/util-debug", "common/promise-debug", "common/statusMap-debug", "common/paymentStatusMap-debug"], function(require, exports, module) {
    var Mywidget = require("common/myWidget-debug"),
        Ajax = require("model/ajax/main-debug"),
        limit = (require("common/handlerbars-debug"), require("common/limit-debug")),
        domUtil = require("common/domUtil-debug"),
        searchButtons = ['<input type="reset" class="fn-btn fn-btn-default fn-btn-sm fn-W60 fn-hide" value="清空" />', '<input type="button" class="fn-btn fn-btn-primary fn-btn-sm fn-W60" value="查询" id="search" />', '<a class="global-link fn-PL10 fn-LH24" id="reset">清空</a>'].join(""),
        statusMap = require("common/statusMap-debug"),
        paymentStatusMap = require("common/paymentStatusMap-debug"),
        advancedQuery = Mywidget.extend({
            ...
            events: {
                'click [data-role="advanced-query"]': function(e) {
                    ...
                    icon.size() > 0 && (down ? (icon.removeClass("fn-advanced-query-down").addClass("fn-advanced-query-up"), me.jQuery("tbody[data-role='advanced-query-conditions'] > [data-role='dosearch']").append(searchButtons), me.jQuery("span[data-role='dosearch']").empty(), me.jQuery("tbody[data-role='advanced-query-conditions']").removeClass("fn-hide"), me.jQuery("tbody[data-role='advanced-query-conditions']").find('select[data-role="class"]').trigger("change")) : (icon.removeClass("fn-advanced-query-up").addClass("fn-advanced-query-down"), me.jQuery("tbody[data-role='advanced-query-conditions']").addClass("fn-hide"), me.jQuery("tbody[data-role='advanced-query-conditions'] > [data-role='dosearch']").empty(), me.jQuery("tbody[data-role='advanced-query-conditions']").find("input:text").val("").prop("disabled", !1), me.jQuery("tbody[data-role='advanced-query-conditions']").find("select").find("option:first").prop("selected", !0), me.jQuery("tbody[data-role='advanced-query-conditions']").find('select[data-role="class"]').trigger("change"), me.jQuery("span[data-role='dosearch']").append(searchButtons)))
                },
                'change [name="type"]': function(e) {
                    var me = this,
                        disputecode = $(e.target).val(),
                        causeAction = me.$('select[name="causeAction"]');
                    return causeAction.length ? void(disputecode ? "payment_order" == disputecode ? domUtil.selectSerialize(causeAction[0], [{
                        key: "申请支付令",
                        value: "申请支付令"
                    }]) : new Ajax({
                        ...
                        domUtil.selectSerialize(causeAction[0], [{
                            key: "不限案由",
                            value: ""
                        }].concat(limit.map(rtv.data, function(val) {
                            return {
                                key: val.parvalue,
                                value: val.parvalue
                            }
                        })))
                    }).submit() : domUtil.selectSerialize(causeAction[0], [{
                        key: "请选择类型再选案由",
                        value: ""
                    }])) : me
                },
                'change [data-role="class"]': function(e) {
                    ...
                    "statusPaymentList" == caseType ? ($(input).prop("placeholder", "案件编号、案号、申请人、被申请人"), domUtil.selectSerialize(type[0], [{
                        key: "支付令",
                        value: "payment_order"
                    }]), status.size() && me.getStatusOptions(status[0], judgeStatus.statusPaymentList, paymentStatusMap), $("#filterCondition label").removeClass("fn-btn-default").addClass("fn-btn-link"), $('#filterCondition [type="radio"]').prop("checked", !1), $("#filterCondition label:first").removeClass("fn-btn-link").addClass("fn-btn-default"), $('#filterCondition [type="radio"]:first').prop("checked", !0), me.hide($("#filterCondition"))) : ($(input).prop("placeholder", "案件编号、案号、原告、被告"), $('[name="type"]').html(me.get("suitTypeHtml")), status.size() && me.getStatusOptions(status[0], judgeStatus.statusSuitList, statusMap), me.show($("#filterCondition"))), $('[name="type"]').trigger("change")
                }
            },
            getStatusOptions: function(statusNode, statusList, statusMap) {
                domUtil.selectSerialize(statusNode, [{
                    key: "全部",
                    value: ""
                }].concat(limit.map(statusList, function(val) {
                    return {
                        key: statusMap[val],
                        value: val
                    }
