define("src/bus/openservice/qd/statistics-debug", ["src/bus/openservice/qd/statistics-hbs-debug", "common/handlerbars-debug", "common/jQuery-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "common/calendar-debug", "model/perSearch/main-debug"], function(require, exports, module) {
    var TEMPLATE = require("src/bus/openservice/qd/statistics-hbs-debug"),
        $ = require("common/jQuery-debug"),
        util = require("common/util-debug"),
        calendar = require("common/calendar-debug"),
        DOWN_URL = "/aeolus/lassenPushDetailRpc/exportPushStatistical.json",
        Persearch = require("model/perSearch/main-debug"),
        defaultStart = util.formatData("yyyy-MM-dd", Date.now() - 6048e5),
        defaultEnd = util.formatData("yyyy-MM-dd", Date.now()),
    ...
    $("#submit-date-fr").val(defaultStart), $("#submit-date-to").val(defaultEnd), startDate.on("selectDate", function(date) {
        endDate.range([date, null]), date.hour(0), date.minute(0), date.second(0)
    }), endDate.on("selectDate", function(date) {
        startDate.range([null, date]), date.hour(23), date.minute(59), date.second(59)
    });
    ...
    $("#generate-btn").on("click", function() {
        ...
        param.pushStartTime = $("#submit-date-fr").val(), param.pushEndTime = $("#submit-date-to").val(), param = JSON.stringify(param), window.open(DOWN_URL + "?filterMap=" + param)
    }), $("#search").on("click", function() {
        perSearchExp.searchListReload()
    })
});
define("src/bus/openservice/qd/statistics-hbs-debug", ["common/handlerbars-debug"], function(require, exports, module) {
    var Handlerbars = require("common/handlerbars-debug"),
        compile = Handlerbars.compile(' <table class="push-data-table fn-table fn-table-data fn-table-td fn-table-border" style="width:100%;margin-top:20px;">    <thead>        <tr>            <th>时间</th>            <th colspan="2">网上法庭推送用户数</th>            <th colspan="2">push通知量</th>            <th colspan="2">push打开量</th>            <th colspan="2">push打开率</th>            <th>push案件量</th>            <th>已完成被告关联</th>            <th>已完成确认送达</th>                    </tr>    </thead>    <tbody>         {{#each this}}        <tr>               <td rowspan="3">{{timeStr}}</td>            <td colspan="2">{{pushuser}}</td>            <td colspan="2">{{pushnotice}}</td>            <td colspan="2">{{pushopen}}</td>            <td colspan="2">{{push_openrate}}</td>            <td rowspan="3">{{pushcases}}</td>            <td rowspan="3">{{pushAssociated}}</td>            <td rowspan="3">{{pushreceived}}</td>        </tr>        <tr>            <td>淘宝</td>            <td>天猫</td>            <td>淘宝</td>            <td>天猫</td>            <td>淘宝</td>            <td>天猫</td>            <td>淘宝</td>            <td>天猫</td>        </tr>        <tr>            <td>{{pushuser_taobao}}</td>            <td>{{pushuser_tmall}}</td>            <td>{{pushnotice_taobao}}</td>            <td>{{pushnotice_tmall}}</td>            <td>{{pushopen_taobao}}</td>            <td>{{pushopen_tmall}}</td>            <td>{{push_openrate_taobao}}</td>            <td>{{push_openrate_tmall}}</td>        </tr>        {{/each}}    </tbody></table>');
    return compile.source = ' <table class="push-data-table fn-table fn-table-data fn-table-td fn-table-border" style="width:100%;margin-top:20px;">    <thead>        <tr>            <th>时间</th>            <th colspan="2">网上法庭推送用户数</th>            <th colspan="2">push通知量</th>            <th colspan="2">push打开量</th>            <th colspan="2">push打开率</th>            <th>push案件量</th>            <th>已完成被告关联</th>            <th>已完成确认送达</th>                    </tr>    </thead>    <tbody>         {{#each this}}        <tr>               <td rowspan="3">{{timeStr}}</td>            <td colspan="2">{{pushuser}}</td>            <td colspan="2">{{pushnotice}}</td>            <td colspan="2">{{pushopen}}</td>            <td colspan="2">{{push_openrate}}</td>            <td rowspan="3">{{pushcases}}</td>            <td rowspan="3">{{pushAssociated}}</td>            <td rowspan="3">{{pushreceived}}</td>        </tr>        <tr>            <td>淘宝</td>            <td>天猫</td>            <td>淘宝</td>            <td>天猫</td>            <td>淘宝</td>            <td>天猫</td>            <td>淘宝</td>            <td>天猫</td>        </tr>        <tr>            <td>{{pushuser_taobao}}</td>            <td>{{pushuser_tmall}}</td>            <td>{{pushnotice_taobao}}</td>            <td>{{pushnotice_tmall}}</td>            <td>{{pushopen_taobao}}</td>            <td>{{pushopen_tmall}}</td>            <td>{{push_openrate_taobao}}</td>            <td>{{push_openrate_tmall}}</td>        </tr>        {{/each}}    </tbody></table>', compile
});
"use strict";
