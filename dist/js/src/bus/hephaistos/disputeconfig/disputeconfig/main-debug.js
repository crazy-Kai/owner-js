"use strict";
define("src/bus/hephaistos/disputeconfig/disputeconfig/main-debug", ["common/jquery-debug", "common/delegate-debug", "src/bus/hephaistos/disputeconfig/disputeconfig/disputeEdit-debug", "common/myWidget-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "src/bus/hephaistos/disputeconfig/disputeconfig/useTpl-debug", "model/modal/main-debug", "common/validator-debug", "common/handlerbars-debug", "model/selectpicker/main-debug", "model/ajax/main-debug", "model/upload/main-debug", "src/bus/hephaistos/disputeconfig/disputeconfig/toConfig-debug", "model/searchList/main-debug"], function(require, exports, module) {
    var $ = require("common/jquery-debug"),
        delegate = require("common/delegate-debug"),
        DisputeEdit = require("src/bus/hephaistos/disputeconfig/disputeconfig/disputeEdit-debug"),
        ToConfig = require("src/bus/hephaistos/disputeconfig/disputeconfig/toConfig-debug"),
        SearchList = require("model/searchList/main-debug"),
        searchListExp = (require("model/ajax/main-debug"), new SearchList({
            element: "#searchList"
        }));
    new DisputeEdit({
        element: "#searchContent",
        trigger: "#addDisputeConfig",
        tpl: "#tpl-disputeconfig",
        paramName: "filterMap"
    ...
    topManager && delegate.on("click", '[data-role="maintain"]', function(e) {
        e.preventDefault();
        var self = $(this);
        topManager.openPage({
            href: self.prop("href"),
...
"use strict";
define("src/bus/hephaistos/disputeconfig/disputeconfig/toConfig-debug", ["common/jquery-debug", "common/myWidget-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "src/bus/hephaistos/disputeconfig/disputeconfig/useTpl-debug", "model/modal/main-debug", "common/validator-debug", "common/handlerbars-debug", "model/selectpicker/main-debug", "model/ajax/main-debug", "model/upload/main-debug"], function(require, exports, module) {
    function checkDeleBtn(element) {
        var deleteNode = element.find('[data-role="delete"]');
        1 === deleteNode.length && deleteNode.hide()
    }

    function filterData(data, valName, paramName, cb1, cb2) {
        var result = {};
        return result.valList = $.map(data[valName], cb1), result.paramList = $.map(data[paramName], cb2), util.log("warn", "before:", data, "after:", result), result
    }

    function toFilterData(val, param) {
        return "competent_court" !== param.configtype ? filterData(val, "lassenDisputeValueVolist", "lassenDisputeParamDolist", function(val) {
            return {
                parcode: val.parcode,
                sort: val.sort,
                configtype: param.configtype,
                parvalue: val.parvalue,
                getConfig: !0,
            ...
            return {
                parcode: val.courtName,
                securityId: val.securityId,
                parvalue: val.courtName
            }
        })
    }
    ...
    var $ = require("common/jquery-debug"),
        ...
        UseTpl = require("src/bus/hephaistos/disputeconfig/disputeconfig/useTpl-debug"),
        Handlerbars = (require("model/modal/main-debug"), require("common/handlerbars-debug")),
        Selectpicker = require("model/selectpicker/main-debug"),
        Ajax = require("model/ajax/main-debug"),
        ToConfig = MyWidget.extend({
            className: "ToConfig",
            attrs: {
            ...
            disputeParam: function(param) {
                ...
                return new Ajax({
                    ...
                    val = toFilterData(val, param), !val.valList.length && val.valList.push({
                        configtype: param.configtype
                    }), new UseTpl({
                        ...
                        events: {
                            'click [data-role="add"]': function(e) {
                                ...
                                Selectpicker.use(node.find('[data-widget="selectpicker"]')), $(e.target).before(node), this.element.find('[data-role="delete"]').show()
                            },
                            'click [data-role="delete"]': function(e) {
                                ...
                                param ? checkDel.call(me, param, node, function() {
                                    checkDeleBtn(self.element)
                                }) : (Selectpicker.dead(node.find('[data-widget="selectpicker"]')), node.closest('[data-target="row"]').remove(), checkDeleBtn(self.element))
                            }
                        }
                    }).after("show", function() {
...
"use strict";
define("src/bus/hephaistos/disputeconfig/disputeconfig/disputeEdit-debug", ["common/jquery-debug", "common/myWidget-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "src/bus/hephaistos/disputeconfig/disputeconfig/useTpl-debug", "model/modal/main-debug", "common/validator-debug", "common/handlerbars-debug", "model/selectpicker/main-debug", "model/ajax/main-debug", "model/upload/main-debug"], function(require, exports, module) {
    ...
    var $ = require("common/jquery-debug"),
        MyWidget = require("common/myWidget-debug"),
        UseTpl = (require("common/util-debug"), require("src/bus/hephaistos/disputeconfig/disputeconfig/useTpl-debug")),
        ...
        Disputeconfig = MyWidget.extend({
            ...
            events: {
                'click [data-role="edit"]': function(e) {
                    var self = $(e.target);
                    this.disputeEdit($.extend(self.data("param"), {
                        title: self.prop("title")
                    }))
                },
            ...
            setup: function() {
                ...
                me.triggerNode && me.delegateEvents(me.triggerNode, "click", function() {
                    initUseTpl.call(me, {
                        title: me.triggerNode.prop("title")
                    })
                })
            },
...
"use strict";
define("src/bus/hephaistos/disputeconfig/disputeconfig/useTpl-debug", ["common/jquery-debug", "common/myWidget-debug", "common/util-debug", "common/promise-debug", "common/limit-debug", "common/limit-dom-debug", "model/modal/main-debug", "common/validator-debug", "common/handlerbars-debug", "model/selectpicker/main-debug", "model/ajax/main-debug", "model/upload/main-debug"], function(require, exports, module) {
    var $ = require("common/jquery-debug"),
        MyWidget = require("common/myWidget-debug"),
        ...
        ModalEditor = MyWidget.extend({
            ...
            attrs: {
                ...
                template: {
                    getter: function() {
                        return this.compileTpl || (this.compileTpl = Handlerbars.compile(this.tpl.html())(this.get("data"))), this.compileTpl
                    },
                    setter: function(val) {
                        this.tpl = $(val)
                    }
            ...
            events: {
                'click [data-role="submit"]': function(e) {
                    var me = this;
                    Validator.oneExecute(me.element, '[data-widget="validator"]') || me.dataPost()
                }
            },
            initProps: function() {},
            setup: function() {
            ...
            destroy: function() {
                var me = this;
                return Upload.dead(me.$('[data-widget="upload"]')), Selectpicker.dead(me.$('[data-widget="selectpicker"]')), ModalEditor.superclass.destroy.call(me), me
            },
            show: function() {
                var me = this;
                return me.element.modal(), me
            },
            hide: function() {
                var me = this;
            ...
            dataPost: function() {
                ...
                return new Ajax({
                    ...
                    Modal.alert(1, msg), me.element.modal("hide"), me.trigger("ajaxSuccess", val, response)
                }).submit(), me
            }
        });
    return ModalEditor
