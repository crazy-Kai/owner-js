"use strict";
define("src/bus/cooperation/allCaseForCooperation/main-debug", ["bus/global/main-debug", "common/jquery-debug", "model/advancedQuery/main-debug", "common/calendar-debug", "model/switchFilterBtn/main-debug", "common/delegate-debug", "common/limit-debug", "common/limit-dom-debug", "common/util-debug", "common/promise-debug", "common/validator-debug", "model/filterConditions/main-debug", "model/modal/main-debug", "common/dialog-debug", "common/domUtil-debug", "common/handlerbars-debug", "model/ajax/main-debug", "model/searchList/main-debug", "transfer-debug", "src/bus/cooperation/allCaseForCooperation/modifyPhone-hbs-debug", "src/bus/cooperation/allCaseForCooperation/sendCode-hbs-debug", "common/statusMap-debug"], function(require, exports, module) {
    function clearOtherDateValue(type) {
        2 != type && 3 != type || ($("#submit-date-fr").val(""), $("#submit-date-to").val(""), $("#submit-date-fr").prop("disabled", "disabled"), $("#submit-date-to").prop("disabled", "disabled")), 1 != type && 3 != type || ($("#register-date-fr").val(""), $("#register-date-to").val(""), $("#register-date-fr").prop("disabled", "disabled"), $("#register-date-to").prop("disabled", "disabled")), 1 != type && 2 != type || ($("#judge-date-fr").val(""), $("#judge-date-to").val(""), $("#judge-date-fr").prop("disabled", "disabled"), $("#judge-date-to").prop("disabled", "disabled"))
    }

    function attachSelectDateEvent(c1, c2, type) {
        c1.on("selectDate", function(date) {
            clearOtherDateValue(type), c2.range(function(aDate) {
                return !(date && !(date && aDate >= date))
            })
        }), c2.on("selectDate", function(date) {
            clearOtherDateValue(type), c1.range(function(aDate) {
                return !(date && !(date && aDate <= date))
            })
        })
    }
    ...
    function getTransferModelByDomAllColNamesAndDomColNames() {
        ...
        return allColNames.length && colNames.length && (allValArr = allColNames.val().split(","), valArr = colNames.val().split(","), model = limit.map(allValArr, function(val, key) {
            var obj = {
                name: val,
                choose: !1
            };
            return limit.contains(valArr, val) && (obj.choose = !0), obj
        })), model
    }
    ...
    function updateDomColNamesValByTransfer() {
        ...
        if (colNames.length && transferExp) {
            var val = limit.map(transferExp.getSelected(), function(val) {
                return "案件" === val.name && (hasCase = !0), val.name
            }).join(",");
            if (!hasCase) return !0;
            colNames.val(val)
        }
    ...
    var $ = require("common/jquery-debug"),
        advancedQuery = require("model/advancedQuery/main-debug"),
        Calendar = require("common/calendar-debug"),
        SwitchFilterBtn = require("model/switchFilterBtn/main-debug"),
        delegate = require("common/delegate-debug"),
        limit = require("common/limit-debug"),
        Validator = (require("common/util-debug"), require("common/validator-debug")),
        Modal = (require("model/filterConditions/main-debug"), require("model/modal/main-debug")),
        ...
        Transfer = require("transfer-debug"),
        modifyPhoneHbs = (require("common/handlerbars-debug"), require("src/bus/cooperation/allCaseForCooperation/modifyPhone-hbs-debug")),
        sendCodeHbs = require("src/bus/cooperation/allCaseForCooperation/sendCode-hbs-debug");
    require("common/statusMap-debug");
    $.ajaxSetup({
        cache: !1
    });
    var c1, c2, c3, c4, c5, c6;
    $("#submit-date-fr").length && (c1 = new Calendar({
        trigger: "#submit-date-fr"
    }), c2 = new Calendar({
        trigger: "#submit-date-to"
    }), attachSelectDateEvent(c1, c2, 1)), $("#register-date-fr").length && (c3 = new Calendar({
        trigger: "#register-date-fr"
    }), c4 = new Calendar({
        trigger: "#register-date-to"
    }), attachSelectDateEvent(c3, c4, 2)), $("#judge-date-fr").length && (c5 = new Calendar({
        trigger: "#judge-date-fr"
    }), c6 = new Calendar({
        trigger: "#judge-date-to"
    ...
    var calendars = {
        ...
        searchListExp = SearchList.use(".searchList", {
            map: function(data) {
                var res = {};
                return data.length && (res.hasRes = !0, res.keys = colNames.val().split(","), res.list = data, res.width = 160 * res.keys.length + 80), res
            }
        });
    new advancedQuery, delegate.on("click", "#search", function() {
        doSearch()
    }), delegate.on("click", "#reset", function() {
        $(":reset").trigger("click"), $.each("#submit-date-fr #submit-date-to #register-date-fr #register-date-to #judge-date-fr #judge-date-to".split(" "), function() {
            calendars[this.toString()] && calendars[this.toString()].trigger("selectDate")
        }), $('[name="type"]').trigger("change"), $("#submit-date-fr, #submit-date-to, #register-date-fr, #register-date-to, #judge-date-fr, #judge-date-to").prop("disabled", "")
    }), new SwitchFilterBtn({
        element: "#filterCondition"
    }).on("switchSuccess", function() {
        doSearch()
    }), new SwitchFilterBtn({
        element: "#filterOrderBy"
    }).on("switchSuccess", function() {
        doSearch()
        ...
        dialogExp = new Dialog({
            ...
            events: {
                'click [data-rule="sure"]': function() {
                    ...
                    0 == val.length ? Modal.alert(0, "请至少选择一列") : updateDomColNamesValByTransfer() ? Modal.alert(0, "列表中必须选择案件") : (searchListExp[0].searchListReload(), this.hide())
                }
            }
        });
                    ...
                    'click [data-role="sure"]': function() {
                        this.validatorExp.execute(function(isErr) {
                            isErr || new Ajax({
                                ...
                                dig.hide(), searchListExp[0].searchListAjax(), Modal.alert(1, rtv)
                            }).submit()
                        })
                    },
            ...
            var dialog = Dialog.show(sendCodeHbs(rtv.data), {
                ...
                events: {
                    ...
                    'click [data-role="send"]': function() {
                        ...
                        }), entityIds = entityidsArray.join(","), batchCode ? new Ajax({
                            request: "/cooperation/lassenCooperationRpc/batchSendAssociateCode.json",
                            param: {
                                entityIds: entityIds
                            }
                        }).on("ajaxSuccess", function(rtv, msg, con) {
                            dialog.hide(), searchListExp[0].searchListAjax(), Modal.alert(1, rtv)
                        }).submit() : Modal.alert(0, "请选择关联码")
                    }
                }
            })
        }).submit()
...
define("src/bus/cooperation/allCaseForCooperation/modifyPhone-hbs-debug", ["common/handlerbars-debug"], function(require, exports, module) {
    var Handlerbars = require("common/handlerbars-debug"),
        compile = Handlerbars.compile(' <form id="modify">    <div class="fn-PaAl10 fn-W450">        <div class="global-tab fn-BBS-ebebeb"><i></i>修改联系人手机号<span class="fn-FS12">(已确认送达的被告无法修改)</span></div>        <div class="fn-MT10">            <table width="100%" class="fn-table fn-table-data fn-table-data-ho">                <thead>                    <tr>                        <th width="160">身份</th>                        <th>手机联系方式</th>                    </tr>                </thead>                <tbody>                    {{#each this}}                        <tr data-serialize-name="list">                            <td>{{identityType}}</td>                            <td>                                <input type="hidden" name="securityId" value="{{securityId}}" />                                <div class="kuma-form-item">                                    <input type="text" name="mobile" value="{{mobile}}" class="fn-input-text fn-input-text-sm fn-W150 kuma-input" {{#if receiveConfirmSuccess}} readOnly="readOnly" {{/if}} data-required="true" data-rule="mobile" data-widget="validator" data-errormessage-required="请填写手机号码" data-errormessage-mobile="请填写正确的手机号码" maxlength="11"/>                                </div>                            </td>                        </tr>                    {{/each}}                </tbody>            </table>            <div class="fn-MT20 fn-ML150">                <input type="button" class="fn-btn fn-btn-primary fn-btn-sm fn-W60" data-role="sure" value="确定" />                <input type="reset" class="fn-btn fn-btn-primary fn-btn-sm fn-W60" data-role="reset" value="重置">            </div>        </div>    </div></form>');
    return compile.source = ' <form id="modify">    <div class="fn-PaAl10 fn-W450">        <div class="global-tab fn-BBS-ebebeb"><i></i>修改联系人手机号<span class="fn-FS12">(已确认送达的被告无法修改)</span></div>        <div class="fn-MT10">            <table width="100%" class="fn-table fn-table-data fn-table-data-ho">                <thead>                    <tr>                        <th width="160">身份</th>                        <th>手机联系方式</th>                    </tr>                </thead>                <tbody>                    {{#each this}}                        <tr data-serialize-name="list">                            <td>{{identityType}}</td>                            <td>                                <input type="hidden" name="securityId" value="{{securityId}}" />                                <div class="kuma-form-item">                                    <input type="text" name="mobile" value="{{mobile}}" class="fn-input-text fn-input-text-sm fn-W150 kuma-input" {{#if receiveConfirmSuccess}} readOnly="readOnly" {{/if}} data-required="true" data-rule="mobile" data-widget="validator" data-errormessage-required="请填写手机号码" data-errormessage-mobile="请填写正确的手机号码" maxlength="11"/>                                </div>                            </td>                        </tr>                    {{/each}}                </tbody>            </table>            <div class="fn-MT20 fn-ML150">                <input type="button" class="fn-btn fn-btn-primary fn-btn-sm fn-W60" data-role="sure" value="确定" />                <input type="reset" class="fn-btn fn-btn-primary fn-btn-sm fn-W60" data-role="reset" value="重置">            </div>        </div>    </div></form>', compile
});
define("src/bus/cooperation/allCaseForCooperation/sendCode-hbs-debug", ["common/handlerbars-debug"], function(require, exports, module) {
    var Handlerbars = require("common/handlerbars-debug"),
        compile = Handlerbars.compile(' <div class="fn-PaAl10 fn-W450">    <div class="global-tab fn-BBS-ebebeb"><i></i>发送关联码</div>    <div class="fn-MT10">        <table width="100%" class="fn-table fn-table-data fn-table-data-ho">            <thead>                <tr>                    <th width="50"><input type="checkbox" data-role="allCheck"/></th>                    <th>身份</th>                    <th width="160">手机联系方式</th>                </tr>            </thead>            <tbody>             {{#each this}}                <tr>                    <td width="40"><input type="checkbox" name="entityIds" data-role="caseCheck" value="{{securityId}}" /></td>                    <td>{{identityType}}</td>                    <td>{{mobile}}</td>                </tr>            {{/each}}            </tbody>        </table>        <div class="fn-MT20 fn-ML150">            <input type="button" class="fn-btn fn-btn-primary fn-btn-sm fn-W60" data-role="send" value="发送" />        </div>    </div></div>');
    return compile.source = ' <div class="fn-PaAl10 fn-W450">    <div class="global-tab fn-BBS-ebebeb"><i></i>发送关联码</div>    <div class="fn-MT10">        <table width="100%" class="fn-table fn-table-data fn-table-data-ho">            <thead>                <tr>                    <th width="50"><input type="checkbox" data-role="allCheck"/></th>                    <th>身份</th>                    <th width="160">手机联系方式</th>                </tr>            </thead>            <tbody>             {{#each this}}                <tr>                    <td width="40"><input type="checkbox" name="entityIds" data-role="caseCheck" value="{{securityId}}" /></td>                    <td>{{identityType}}</td>                    <td>{{mobile}}</td>                </tr>            {{/each}}            </tbody>        </table>        <div class="fn-MT20 fn-ML150">            <input type="button" class="fn-btn fn-btn-primary fn-btn-sm fn-W60" data-role="send" value="发送" />        </div>    </div></div>', compile
});
"use strict";
