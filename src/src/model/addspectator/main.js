...
define(function(require, exports, module) {
    ...
    var addSpectatorHbs = require('./spectator-hbs'),
        templateSeedNode = require('./spectatorSeed-hbs');
    ...
    var AddSpectator = MyWidget.extend({
        ...
        show: function(param){
            var me = this;
            var dialog = Modal.show( addSpectatorHbs(param), {
                width: 650,
                events: {
                    // 新增
                    'click [data-role="save"]': function(){
                        // 验证
                        if( !Validator.oneExecute(dialog.element, '[data-widget="validator"]') ){
                            save.call(me, dialog);
                        };
                    },
                    // 删除
    ...
    function check(dialog){
        ...
        if(dialog.$('[data-role="delete"]').length >= me.get('size')){
            dialog.$('[data-role="save"]').hide();
        }else{
            dialog.$('[data-role="save"]').show();
        };
    };
