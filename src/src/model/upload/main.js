	...
	var TEMPLATE_WAITE = handlerbars.compile([
		...
		TEMPLATE_ONE = handlerbars.compile([
			'<span class="file-list JS-target-item fn-VATop" data-id="{{id}}">',
    			'<input type="checkbox" checked="checked" name="UploadFileInput" value="{{id}}" style="display:none;"/><a class="file-list-text fn-btn-link" {{#if imgView}}data-rule="imgView"{{/if}} href="{{url}}" target="_blank">{{name}}</a> <span class="fn-CuPo kuma-icon kuma-icon-close JS-trigger-click-remove" aria-hidden="true"></span>',
    		'</span>',
		].join('')),
