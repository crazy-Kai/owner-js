...
define(function(require, exports, module) {
            ...
            validatorExp.execute(function(err, errlist){
                if(err){
                    util.log(errlist);
                     ...
                     // 验证手机&邮箱二者选一
                    var DO = domUtil.serialize('#updatesInformation-form'),
                        lassenSuitEntityDo = DO.lassenSuitEntityDo;
                    var needCheck = ($('[name="lassenSuitEntityDo.mobile"]:visible').size()>0 && $('[name="lassenSuitEntityDo.email"]:visible').size()>0)
                    if( needCheck && !lassenSuitEntityDo.mobile && !lassenSuitEntityDo.email ){
                        return Modal.alert(0, '手机号码或者邮箱必须填写一项。');
                    }else if( lassenSuitEntityDo.legalRepresent === "" ){
                        return Modal.alert(0, '法定代表人不能为空，请进入个人中心填写法定代表人。');
                    }
    ...
    chooseTypeNext.on('click', function(){
        new Ajax({
            ...
            domUtil.redirect( chooseTypePage.find('[type="radio"]:checked').data('jump')+'?securityId='+ domUtil.getEscapeUrl(rtv.securityId) );
    ...
    delegate.on('change', '[data-identifytype="agentDetails"] [name="relationCode"]', function(e){
        console.log('relationCode change');
        var type = $(e.target).val();
