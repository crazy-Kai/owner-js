	...
	var NewMysuitNav = React.createClass({
		...
		render: function(){
			var me = this,
				...
				propsNav = state.nav;
			document.title = propsNavMap[state.navKey] || state.navKey || '';
			// 格式化数据
			return (
				<div className="sc-nav sc-left">
		            <dl className="sc-MaTo10">
		            	{ 
		            		limit.map(propsNav, function(val, index){
				        		return limit.map(val, function(val, key){
				        			return !key ? 
				        				<dt key={key}>{propsNavMap[val.key]}</dt> : 
				        				<dd key={key}>
				        					<a target={val.isForward === 'true' ? '_blank' : state.iframeName} 
				        						ref={(!index && key ===1) ? 'first' : null} 
				        						href={val.val} 
				        						onClick={NewMysuitController.Actions.targetNav} 
				        						className={ (state && state.navKey === val.key) ? "ch-active" : '' } 
				        						data-key={val.key}>{propsNavMap[val.key]}</a>
				        				</dd>
				        		})
				        	})
			        	}
