...
define(function(require, exports, module) {
	...
	var TEMPLATE_WAITE = handlerbars.compile([
		...
		TEMPLATE_ONE = handlerbars.compile([
			'<span class="file-list JS-target-item fn-VATop" data-id="{{id}}">',
    			'<input type="checkbox" checked="checked" name="UploadFileInput" value="{{id}}" style="display:none;"/><a class="file-list-text fn-btn-link" {{#if imgView}}data-rule="imgView"{{/if}} href="{{url}}" target="_blank" title="{{name}}">{{wrapWord name 20}}</a> <span class="fn-CuPo kuma-icon kuma-icon-close JS-trigger-click-remove" aria-hidden="true"></span>',
    		'</span>',
		].join('')),
	...
	var Upload = MyWidget.extend({
		...
		attrs: {
			...
			imgView: false,
			size: 10,
			fileSize: 15728640 //15m * 1024k * 1024b
		},
		//事件
		events: {
		...
		initUpload: function(){
			...
			return new nwUpload({
				...
				change: function(e){
					...
					if(fileSize && fileSize > me.get('fileSize')){
						return Modal.alert(0, '文件大小限制在15M以内');
					};
				...
				error: function(e){
					if(e.statusText){
						if(e.statusText === 'Request Entity Too Large'){
							Modal.alert(0, '文件大小限制在15M以内');
						}else{
							Modal.alert(0, e.statusText);
						};
					}else{
						me.log(e.statusText);
					};
		...
		uploadToggle: function(){
			var me = this,
				...
				size = me.get('size');
			// console.log(nwUpload, me.$('.JS-target-item').length, size, me.$('.file-list').length >= size);
			if(me.$('.file-list').length >= size){
