...
define(function(require, exports, module) {
	...
	var $ = EditAccus.jQuery,
		...
		orderTemplate = [
			'{{#if this}}',
			'{{#each this}}',
			'<tr {{#isEqual isDisputeOrder "y"}}class="JS-need-tip" style="color:#ccc;" data-content="该订单已提起过诉讼，请勿重复提起"{{/isEqual}}>',
				'<td>',
					'<input {{#isEqual isDisputeOrder "y"}}class="fn-btn-disabled"{{/isEqual}} type="radio" name="orderId" value="{{orderId}}" {{#isEqual isDisputeOrder "y"}}disabled{{/isEqual}}/>',
					'<input type="radio" class="fn-hide-input" name="bizOrderId" value="{{orderId}}" />',
					'<input type="radio" class="fn-hide-input" name="parentOrderId" value="{{parentOrderId}}" />',
			...
			me.soOrderDialog = new Dialog({
				...
				events: {
					...
					'click tr': function(e){
						...
						target.find(':disabled').prop('checked',false);
					},
					//改变关联账号
				...
				for(var i in tips) {
					tips[i].after('show', function() {
						$(this.element[0]).css({'zIndex':9999});
					})
				}
				// console.log(tips);
			}).render();
